...
void callback(mozart_event event, void *param)
{
	static int id = 0;
	switch (event.type) {
	case EVENT_KEY:
		printf("key event arrived.\n");
		printf("id: %d, keytype: %u, keycode: %s, keyvalue: %s\n", id++,
		       event.event.key.key.type,
		       keycode_str[event.event.key.key.code],
		       keyvalue_str[event.event.key.key.value]);
		break;
	case EVENT_MISC:
		printf("misc event arrived.\n");
		printf("%s %s.\n", event.event.misc.name, event.event.misc.type);
		break;
	default:
		printf("Unknown protocol event arrived.\n");
		break;
	}
...
int main(void)
{
	...
	handler = mozart_event_handler_get(callback, NULL);
