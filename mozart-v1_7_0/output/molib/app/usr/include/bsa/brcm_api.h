...
typedef struct
{
    ...
    tBTM_BLE_ADV_TX_POWER   tx_power;               /* adv tx power */

}tBTM_BLE_ADV_PARAMS;
/* BSA_SPECIFIC */
#if defined(BRCM_PRM_CONST_PATCHRAM_DEF) && (BRCM_PRM_CONST_PATCHRAM_DEF == TRUE)
extern const UINT8 brcm_patchram[];
...
**
*******************************************************************************/
BT_API extern BOOLEAN BRCM_PrmInit (tBRCM_PRM_CBACK *p_cb,
                                       BOOLEAN use_internal_patch_data,
                                       UINT32 address, UINT8 file_type);
