...
#define ADDQUE(xi,xq,in)\
  ...
  uint32_t		ci    = channel; 	// Index for channels
  uint32_t		nch   = channel;   	// Number of channels
  uint32_t		inc   = s->up/s->dn;
  uint32_t		level = s->up%s->dn;
  uint32_t		up    = s->up;
  uint32_t		dn    = s->dn;
  uint32_t		ns    = inputlen/(bitspersample>>3);
  ...
  while(ci--){
    ...
    register FORMAT*	in    = ((FORMAT*)inbuff)+ci;
    register FORMAT*	out   = ((FORMAT*)outbuff)+ci;
