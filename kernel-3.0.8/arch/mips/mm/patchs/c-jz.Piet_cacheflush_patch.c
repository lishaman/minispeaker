 ...
 * ----------------------------
 * Selective flushing of a short range of addresses with protected_blast_other_cpu_dcache_range_ipi()
 * now appears to be stable with the addition of disabling preemption and 2x ro 3x faster we browser
 * than flushing the complete dcache and icache on other CPUs with local_r4k_flush_dcache_jz_ipi()
 * and local_r4k_flush_icache_jz_ipi(). running flushtest is stable 4 runs (and counting).
 *
 ...
 *
 * Processor must be preempted for cache flush operations. This will be done
 * by default by r4k_on_other_cpu() and r4k_on_each_cpu() but also needs to be
 * done for the local processor if invoked directly.
 *
 * sys_cacheflush() {
 ...
 * }
 *
 * Could add preempt_disable() and preempt_enable() around paths not using
 * r4k_on_other_cpu() and r4k_on_each_cpu() but it's simpler to just nest the calls
 * preempt_disable() and preempt_enable().
 *
 * Expect to do so more clean up after this is well tested and committed.
 */
static void r4k_flush_icache_range(unsigned long start, unsigned long end)
{
	preempt_disable();
