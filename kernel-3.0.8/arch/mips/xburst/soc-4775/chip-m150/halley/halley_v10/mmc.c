...
struct jzmmc_platform_data inand_pdata = {
	...
	.capacity  			= MMC_CAP_SD_HIGHSPEED | MMC_CAP_MMC_HIGHSPEED | MMC_CAP_8_BIT_DATA | MMC_CAP_NONREMOVABLE,
...
struct jzmmc_platform_data sdio_pdata = {
	...
	.ocr_avail			= MMC_VDD_20_21 | MMC_VDD_21_22,
#else
	.ocr_avail			= MMC_VDD_29_30 | MMC_VDD_30_31,
#endif
...
static void wifi_le_set_io(void)
{
	...
	/*set WL_MSC2_D0~WL_MSC2_D3,
	WL_MSC2_CLK, WL_MSC2_CMD pins to output low statu*/
}
...
static void wifi_le_restore_io(void)
{
	...
	/*set WL_MSC2_D0~WL_MSC2_D3,
	WL_MSC2_CLK, WL_MSC2_CMD pins to GPIO_FUNC_0*/
}
...
int iw8101_wlan_init(void)
{
	...
	gpio_request(power_en, "wifi_pw_en");
...
#ifdef GPIO_WIFI_RST_N
	int reset = iw8101_data.wifi_reset;
#endif
	printk("IW8101_wlan_power_on => pwer %p \n", power);
