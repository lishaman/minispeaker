...
{
	static struct wake_lock	*wifi_wake_lock = &rtw_8189fs_data.wifi_wake_lock;
	int reset;
	...
	rtw_8189fs_data.wifi_reset = reset;
...
int rtw_wlan_power_on(int flag)
{
	static struct wake_lock	*wifi_wake_lock = &rtw_8189fs_data.wifi_wake_lock;
	int reset = rtw_8189fs_data.wifi_reset;
...
int rtw_wlan_power_off(int flag)
{
	static struct wake_lock	*wifi_wake_lock = &rtw_8189fs_data.wifi_wake_lock;
	int reset = rtw_8189fs_data.wifi_reset;
...
static ssize_t rtw_8189fs_manual_set(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
        int ret = 0;
        /* rlt8189fs default state is power on */
        static int init_state = 1;

        if (strncmp(buf, "0", 1) == 0) {
                if (!init_state)
                        goto out;
                ret = rtw_wlan_power_off(NORMAL);
                init_state = 0;
        } else if (strncmp(buf, "1", 1) == 0) {
                if (init_state)
                        goto out;
                ret = rtw_wlan_power_on(NORMAL);
                init_state = 1;
        } else
                ret = -1;
        if (ret)
                pr_warn("===>%s realtek 8189fs power ctl fail\n", __func__);
...
static DEVICE_ATTR(manual, S_IWUSR | S_IRGRP | S_IROTH,
                   NULL, rtw_8189fs_manual_set);

int rtw_8189fs_manual_sysfs_init(struct device *dev)
{
        int ret = 0;
...
EXPORT_SYMBOL(rtw_8189fs_wlan_init);
EXPORT_SYMBOL(rtw_8189fs_manual_sysfs_init);
EXPORT_SYMBOL(rtw_wlan_power_on);
EXPORT_SYMBOL(rtw_wlan_power_off);
