...
static struct wifi_platform_data realtek_wlan_pdata;
...
static struct platform_device wlan_device = {
	...
	.id     = 1,
	.dev    = {
		.platform_data = &realtek_wlan_pdata,
	},
...
{
	int ret;
	memset(&realtek_wlan_pdata, 0, sizeof(realtek_wlan_pdata));

	realtek_wlan_pdata.get_mac_addr = get_wifi_mac_addr;
...
MODULE_DESCRIPTION("Realtek wlan driver");
MODULE_LICENSE("GPL");
