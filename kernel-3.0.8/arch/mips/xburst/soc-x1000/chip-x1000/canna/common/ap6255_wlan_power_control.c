...
{
	static struct wake_lock	*wifi_wake_lock = &bcm_ap6255_data.wifi_wake_lock;
	int reset;
	...
	bcm_ap6255_data.wifi_reset = reset;
...
{
	static struct wake_lock	*wifi_wake_lock = &bcm_ap6255_data.wifi_wake_lock;
	int reset = bcm_ap6255_data.wifi_reset;
...
static ssize_t bcm_ap6255_manual_set(struct device *dev,
				...
				const char *buf, size_t count)
{
	...
	if (strncmp(buf, "0", 1) == 0) {
		...
		ret = -1;
	if (ret)
		pr_warn("===>%s bcm_ap6255 power ctl fail\n", __func__);
...
int get_host_wake_oob(void)
{
	int host_oob_irq = 0;
	gpio_request(GPIO_WIFI_WAKE, "oob_irq");
	gpio_direction_input(GPIO_WIFI_WAKE);
	host_oob_irq = gpio_to_irq(GPIO_WIFI_WAKE);

	return host_oob_irq;
}

EXPORT_SYMBOL(bcm_ap6255_wlan_init);
EXPORT_SYMBOL(bcm_ap6255_manual_sysfs_init);
EXPORT_SYMBOL(bcm_wlan_power_on);
EXPORT_SYMBOL(bcm_wlan_power_off);
EXPORT_SYMBOL(get_host_wake_oob);
