...
struct jz_aic {
	...
	/*for subdev*/
	struct jz_aic_subdev_pdata subdev_pdata;		/*aic/spdif/ac97 use the same pdata*/
	struct platform_device *psubdev_i2s;
	struct platform_device *psubdev_spdif;
	struct platform_device *psubdev_ac97;
};
		...
		unsigned int val)
{
	...
	//dev_dbg(dev, "write val %x reg %x\n", val , jz_aic->vaddr_base + reg);
	writel(val, jz_aic->vaddr_base + reg);
}

static unsigned int inline jz_aic_read_reg(struct device *dev, unsigned int reg)
{
	struct jz_aic *jz_aic = dev_get_drvdata(dev);

	//printk("readl %x\n", jz_aic->vaddr_base + reg);
	return readl(jz_aic->vaddr_base + reg);
}
