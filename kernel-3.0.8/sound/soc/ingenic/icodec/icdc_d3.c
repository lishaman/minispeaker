...
static int icdc_d3_digital_mute(struct snd_soc_dai *dai, int mute)
{
...
static const struct snd_kcontrol_new icdc_d3_mercury_vmux_controls =
	SOC_DAPM_ENUM_VIRT("Route", icdc_d3_enum[1]);
...
static int icdc_d3_probe(struct snd_soc_codec *codec)
{
	...
	snd_soc_update_bits(codec, SCODA_REG_ICR, SCODA_ICR_INT_FORM_MASK, SCODA_ICR_INT_FORM_HIGH);
...
static ssize_t icdc_d3_regs_store(struct device *dev,
...
static struct device_attribute icdc_d3_sysfs_attrs =
	__ATTR(hw_regs, S_IRUGO|S_IWUGO, icdc_d3_regs_show, icdc_d3_regs_store);
...
static int icdc_d3_platform_probe(struct platform_device *pdev)
{
	...
	icdc_d3->irqno = res->start;
	icdc_d3->irqflags = res->flags &
