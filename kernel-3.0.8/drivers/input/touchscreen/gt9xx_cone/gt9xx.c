...
static const char *goodix_small_ts_name = "Goodix Capacitive TouchScreen(Small)";
...
static struct input_dev *last_idev = NULL;
...
#if GTP_ICS_SLOT_REPORT
	input_mt_slot(last_idev, id);
	input_report_abs(last_idev, ABS_MT_TRACKING_ID, -1);
	GTP_DEBUG("Touch id[%2d] release!", id);
#else
	input_report_abs(last_idev, ABS_MT_TOUCH_MAJOR, 0);
	input_report_abs(last_idev, ABS_MT_WIDTH_MAJOR, 0);
	input_mt_sync(last_idev);
#endif
}
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    input_report_abs(idev, ABS_MT_TOUCH_MAJOR, w);
...
// printk("===>pre_touch:%02x, finger:%02x, touch_num:%d\n", pre_touch, finger, touch_num);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    /* small tp */
    ts->small_input_dev = input_allocate_device();
    if (ts->small_input_dev == NULL) {
	    GTP_ERROR("Failed to allocate small input device\n");
	    return -ENOMEM;
    }

    ts->small_input_dev->evbit[0] = BIT_MASK(EV_SYN) | BIT_MASK(EV_KEY) | BIT_MASK(EV_ABS) ;
#if GTP_ICS_SLOT_REPORT
    __set_bit(INPUT_PROP_DIRECT, ts->small_input_dev->propbit);
    input_mt_init_slots(ts->small_input_dev, 16);     // in case of "out of memory"
#else
    ts->small_input_dev->keybit[BIT_WORD(BTN_TOUCH)] = BIT_MASK(BTN_TOUCH);
#endif

#if GTP_SLIDE_WAKEUP
    input_set_capability(ts->small_input_dev, EV_KEY, KEY_POWER);
#endif

    input_set_abs_params(ts->small_input_dev, ABS_MT_POSITION_X, 0, ts->abs_x_max, 0, 0);
    input_set_abs_params(ts->small_input_dev, ABS_MT_POSITION_Y, 0, 30, 0, 0);
    input_set_abs_params(ts->small_input_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(ts->small_input_dev, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(ts->small_input_dev, ABS_MT_TRACKING_ID, 0, 255, 0, 0);

    sprintf(phys, "input/ts");
    ts->small_input_dev->name = goodix_small_ts_name;
    ts->small_input_dev->phys = phys;
    ts->small_input_dev->id.bustype = BUS_I2C;
    ts->small_input_dev->id.vendor = 0xDEAD;
    ts->small_input_dev->id.product = 0xBEEF;
    ts->small_input_dev->id.version = 10427;

    ret = input_register_device(ts->small_input_dev);
    if (ret)
    {
	    GTP_ERROR("Register %s input device failed", ts->small_input_dev->name);
	    return -ENODEV;
    }
 ...
 static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ts->vcc_reg = regulator_get(NULL, "vtsc");
    if (IS_ERR(ts->vcc_reg)) {
	    kfree(ts);
	    GTP_ERROR("failed to get VCC regulator.");
	    return -EINVAL;
    }
