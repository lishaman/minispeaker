...
static inline unsigned int get_phy_addr(unsigned long vaddr)
{
    ...
    )
{
    ...
    //dma_cache_wback_inv((unsigned long) Logical, 0x200000);

#if 0
     /* flush Physical address */
    dma_sync_single_for_device(
              gcvNULL,
              (dma_addr_t)get_phy_addr((unsigned long)Logical),
              Bytes,
              DMA_BIDIRECTIONAL);
#endif
    ...
    )
{
    ...
    //printk("gckOS_SetGPUPower() Os=0x%X Core=%d Clock=%d Power=%d\n", Os, Core, Clock, Power);
    {
        ...
        if ( !device
             || IS_ERR(device->clk_pwc_gpu)
             || IS_ERR(device->clk_gpu)
             || IS_ERR(device->clk_cgu_gpu)
             ) {
            printk("gpu clock get error: !device\n");
            goto end;
        }
        //printk("Power_ON=%d, Clock_ON=%d\n", device->Power_ON, device->Clock_ON);
        //dump_m200_gpu_clock();

        if (!device->Power_ON && Power) {
            clk_enable(device->clk_pwc_gpu);
            device->Power_ON = 1;
        }

        if (!device->Clock_ON && Clock) {
            clk_enable(device->clk_gpu);
            clk_enable(device->clk_cgu_gpu);
            device->Clock_ON = 1;
        }
        else {
            clk_disable(device->clk_cgu_gpu);
            clk_disable(device->clk_gpu);
            device->Clock_ON = 0;
        }

        if (device->Power_ON && !Power) {
            clk_disable(device->clk_pwc_gpu);
            device->Power_ON = 0;
        }
...
#endif /* ENABLE_GPU_CLOCK_BY_DRIVER && (defined(CONFIG_SOC_4785) || defined(CONFIG_SOC_M200)) */
