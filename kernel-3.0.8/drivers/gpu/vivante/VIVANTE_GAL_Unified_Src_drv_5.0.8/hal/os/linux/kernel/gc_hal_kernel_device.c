    ...
    )
{
...
#if ENABLE_GPU_CLOCK_BY_DRIVER && defined(CONFIG_SOC_M200) && defined(CONFIG_GPU_DYNAMIC_CLOCK_POWER)
    {
        ...
        device->clk_pwc_gpu = clk_get(NULL, "pwc_gpu"); /* gpu power */
        device->clk_gpu = clk_get(NULL, "gpu");
        device->clk_cgu_gpu = clk_get(NULL, "cgu_gpu");
        if (IS_ERR(device->clk_pwc_gpu)) {
            printk("gpu clock get error: device->clk_pwc_gpu\n");
        }
        if (IS_ERR(device->clk_gpu)) {
            printk("gpu clock get error: device->clk_gpu\n");
        }
        if (IS_ERR(device->clk_cgu_gpu)) {
