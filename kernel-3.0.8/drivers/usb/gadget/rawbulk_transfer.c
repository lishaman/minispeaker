...
static int start_upstream(struct upstream_transaction *t, gfp_t mem_flags) {
	...
	usb_fill_bulk_urb(t->urb, transfer->udev,
			usb_rcvbulkpipe(transfer->udev,
				get_epnum(transfer->upstream.host_ep)),
	...
	usb_mark_last_busy(transfer->udev);
	if (rc < 0) {
...
static void downstream_second_stage(struct usb_ep *ep,
	...
	do {
		...
		urb->status = 0;
		usb_fill_bulk_urb(urb, transfer->udev,
				usb_sndbulkpipe(transfer->udev,
					get_epnum(transfer->downstream.host_ep)),
...
int rawbulk_forward_ctrlrequest(struct rawbulk_transfer *transfer, const struct usb_ctrlrequest *ctrl) {
	...
	transfer->forward_dr.bRequestType = ctrl->bRequestType;
	transfer->forward_dr.bRequest = ctrl->bRequest;
	transfer->forward_dr.wValue = ctrl->wValue;
	transfer->forward_dr.wIndex = 0;//MODEM DATA PORT
	transfer->forward_dr.wLength = ctrl->wLength;

	if (!transfer->forwarding_urb) {
		urb = usb_alloc_urb(0, GFP_KERNEL);
		if (!urb)
			return -ENOMEM;
		transfer->forwarding_urb = urb;
	} else
...
int rawbulk_start_transactions(struct rawbulk_transfer *transfer, int nups, int ndowns, int upsz,
	...
	if (!transfer->cdev || !transfer->udev)
		return -ENODEV;
	...
	ldbg("start transactions on id %d, nups %d ndowns %d upsz %d downsz %d split %d pushable %d\n",
			transfer->id, nups, ndowns, upsz, downsz, splitsz, pushable);
	...
	if (suspended) {
		ldbg("interface %d is sleeping", transfer->id);
		mutex_unlock(&transfer->mutex);
		return 0;
	}
...
int rawbulk_transfer_state(struct rawbulk_transfer *transfer) {
	...
	if (!transfer->udev || !transfer->cdev)
		return -ENODEV;
...
int rawbulk_transfer_statistics(struct rawbulk_transfer *transfer, char *buf) {
	...
	if (transfer->udev)
		pbuf += sprintf(pbuf, " host device: %d-%d\n", transfer->udev->bus->busnum,
				transfer->udev->devnum);
	else
		pbuf += sprintf(pbuf, " host device: -ENODEV\n");
	if (transfer->cdev && transfer->cdev->config)
		pbuf += sprintf(pbuf, " gadget device: %s\n",
				transfer->cdev->config->label);
	else
...
int rawbulk_bind_transfer(struct rawbulk_transfer *transfer, struct usb_function *function, struct
	...
	transfer->cdev = function->config->cdev;
