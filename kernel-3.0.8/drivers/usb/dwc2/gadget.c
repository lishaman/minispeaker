...
static void dwc2_gadget_start_in_transfer(struct dwc2_ep *dep) {
	...
	if (req->trans_count_left == 0) {
		...
		int n, sp;
                int trans_count = req->trans_count_left;

                if (req->trans_count_left > DWC2_DEP_ALIGN_ALLOC_SIZE &&
                    (unsigned int)(req->request.buf) % 4)
                        trans_count = DWC2_DEP_ALIGN_ALLOC_SIZE;

		n = trans_count / dep->maxp;
		if (n > dep->mc)
			n = dep->mc;

		pktcnt = n;

		sp = trans_count - n * dep->maxp;
		if ( (sp > 0) && (n == dep->mc)) {
			sp = 0;
		}
		...
		int (*bind)(struct usb_gadget *))
{
	...
	dev_info(dwc->dev, "bind to driver %s %s, %d\n", driver->driver.name, __func__,__LINE__);
	ret = bind(&dwc->gadget);
	if (ret) {
