...
int update_slcd_frame_buffer(void)
{
	...
	rgb565_to_gray(frm_buf_addr, cur_buf_addr, cur_epd_mode);
...
static void rgb565_to_gray(unsigned char *lcd_frame, unsigned char *epd_frame, int epdmode)
{
	...
	w = panel_info.epd_var.yres;
	h = panel_info.epd_var.xres;
	int write_region[4] = {0,0, w, h};

	bpp = panel_info.epd_var.bits_per_pixel;
	if ( bpp == 18 || bpp == 24)
		bpp = 32;
	if ( bpp == 15 )
		bpp = 16;
...
static int jz4775fb_map_smem(struct device *device)
{
	unsigned long page;
	...
	uint32_t snp_buf_smem	= 0;
	uint32_t cur_buf_smem	= 0;
	...
	current_a2buffer	= kzalloc(cur_buf_size, GFP_KERNEL);
	waveform_buffer     = kzalloc(wfm_buf_size, GFP_KERNEL);
	working_buffer		= kzalloc(wrk_buf_size, GFP_KERNEL);
	wfm_lut_buf			= kzalloc(WFM_LUT_SIZE, GFP_KERNEL);
	current_buffer		= dma_alloc_coherent(device, cur_buf_size * CURBUF_NUM, &cur_buf_smem, GFP_KERNEL | __GFP_ZERO);
	snooping_buffer		= dma_alloc_coherent(device, snp_buf_size, &snp_buf_smem, GFP_KERNEL | __GFP_ZERO);
	...
	REG_DDR_SNP_LOW = (snp_buf_smem & 0xfffff000);
	REG_DDR_SNP_HIGH = (snp_buf_smem & 0xfffff000);
#else
	REG_DDR_SNP_LOW = ((snp_buf_smem >> 12) & 0xfffff);
	REG_DDR_SNP_HIGH = ((snp_buf_smem >> 12) & 0xfffff);
#endif
	dprintk("REG_DDR_SNP_LOW = 0X%08X\n", REG_DDR_SNP_LOW);
	dprintk("REG_DDR_SNP_HIGH = 0X%08X\n", REG_DDR_SNP_HIGH);
...
static int __init jz4775fb_probe(struct platform_device *pdev)
{
	...
	sprintf(jz_epd->clk_name, "epd");
#ifdef CONFIG_SOC_M200
	sprintf(jz_epd->pclk_name, "cgu_lpc");
#else
