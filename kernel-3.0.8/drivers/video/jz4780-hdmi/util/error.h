...
typedef enum
{
	...
	ERR_PRODUCT_INFO_NOT_PROVIDED, /* Non fatal, SPD (and VSD if no 3D) were not configured */

	ERR_MEM_ALLOCATION, /* Dynamic memory allocated for HDCP functionality failed - FATAL ERROR */
	ERR_HDCP_MEM_ACCESS, /* Memory where runtime HDCP information are written by HDCP engine cannot be accessed - FATAL ERROR */
	ERR_HDCP_FAIL,
	...
	ERR_PIXEL_REPETITION_NOT_SUPPORTED, /* Pixel repetition not supported - FATAL ERROR*/
	ERR_COLOR_DEPTH_NOT_SUPPORTED, /* Color depth not supported - FATAL ERROR*/

	ERR_DVI_MODE_WITH_PIXEL_REPETITION, /* DVI does not support packets so it cannot tell if there is pixel repetition - FATAL ERROR */
	ERR_CHROMA_DECIMATION_FILTER_INVALID, /* an invalid filter chosen - FATAL ERROR */
	ERR_CHROMA_INTERPOLATION_FILTER_INVALID, /* an invalid filter chosen - FATAL ERROR */
	ERR_COLOR_REMAP_SIZE_INVALID, /* an invalid YCC422 remap size - FATAL ERROR */
	ERR_INPUT_ENCODING_TYPE_INVALID, /* check video parameters for encoding type enumerator - FATAL ERROR*/
	ERR_OUTPUT_ENCODING_TYPE_INVALID, /* check video parameters for encoding type enumerator - FATAL ERROR*/
