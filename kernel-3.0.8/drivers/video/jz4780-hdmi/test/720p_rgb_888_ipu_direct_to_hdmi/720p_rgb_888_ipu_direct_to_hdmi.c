...
int main(int argc, char *argv[])
{
	...
	if (ioctl(fb_fd1, JZFB_SET_ALPHA, &fg0_alpha) < 0) {
		printf("set fg 0 global alpha 0xff fail\n");
		return -1;
	}
	printf("set fg 0 global alpha success\n");
...
//		frameCount++;
	...
	printf("set fg 0 original global alpha success\n");
	...
	if (ioctl(hdmi_fd, HDMI_POWER_OFF, 0) < 0) {
		printf("HDMI_POWER_OFF\n\n");
		return -1;
	}
