...
void halFrameComposerDebug_ForceVideo(u16 baseAddr, u8 bit)
{
	...
	if (bit != 0)
	{
		...
		access_CoreWriteByte(bit ? 0xff : 0x00, baseAddr + FC_DBGTMDS1); /* G */
	...
	{
		access_CoreWrite(bit, baseAddr + FC_DBGFORCE, 0, 1);
		access_CoreWriteByte(bit ? 0xFF : 0x00, baseAddr + FC_DBGTMDS2); /* R */
		access_CoreWriteByte(bit ? 0x00 : 0x00, baseAddr + FC_DBGTMDS1); /* G */
		access_CoreWriteByte(bit ? 0x00 : 0x00, baseAddr + FC_DBGTMDS0); /* B */
	}
}
