...
int jpeg2rle (char *filename, char *outfilename)
{
	...
	fwrite (&cinfo.output_width, 2, 1, outfile); //´æ´¢Í¼Æ¬¿í¶È
	fwrite (&cinfo.output_height, 2, 1, outfile); //´æ´¢Í¼Æ¬¸ß¶È
...
int png2rle(char *infilename, char *outfilename)
{
		...
		fwrite (&width, 2, 1, outfile); //´æ´¢Í¼Æ¬¿í¶È
	    fwrite (&height, 2, 1, outfile); //´æ´¢Í¼Æ¬¸ß¶È
		...
		for(; i < height; i++)
		{
				for(j = 0; j < (4 * width); j += 4)
				{
						...
						color = to565( row_pointers[i][j],row_pointers[i][j + 1],row_pointers[i][j + 2]);
						if (count) {
