...
static long jzdmic_ioctl(struct file *filp, unsigned int cmd, unsigned long args)
{
	...
	switch(cmd) {
		case DMIC_SET_CHANNEL:
			printk("dmic set channels\n");
			copy_from_user(&tmp, argp, sizeof(long));
			ret = wakeup_module_ioctl(DMIC_IOCTL_SET_CHANNEL, tmp);
			break;
		case DMIC_SET_SAMPLERATE:
			printk("dmic set samplerate\n");
			copy_from_user(&tmp, argp, sizeof(long));
			ret = wakeup_module_ioctl(DMIC_IOCTL_SET_SAMPLERATE, tmp);
			break;
