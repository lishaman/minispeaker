...
void build_one_desc(struct dma_config* dma, struct dma_desc* _desc, struct dma_desc* next_desc){
	...
	desc->drt = dma->type;
	desc->sd = dma->sd;
	desc->dcm = dma->increment<<22 | dma->tsz<<8 | dma->sp_dp<<12 \
		| dma->rdil<<16 |dma->stde<<2| /*dma->tie<<1 |*/ dma->link;
}
...
void pdma_config(struct dma_config* dma)
{
	if(!dma->descriptor){
		...
		REG_DMADCM(dma->channel) = dma->increment<<22 | dma->tsz<<8 \
			| dma->rdil<<16 | dma->sp_dp<<12|dma->stde<<2| /*dma->tie<<1 |*/ dma->link ;
	}else{
