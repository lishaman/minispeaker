/*
 *  stk220x_int.c - Linux kernel modules for stk220x ambient light sensor with interrupt
 *
...
#include <linux/stk/stk_defines.h>
#include <linux/stk/stk_lk_defs.h>
#include "lsensor_stk220x.h"

#define ADDITIONAL_GPIO_CFG 1

#define STKALS_DRV_NAME	"lsensor_stk220x"
			...
			const struct i2c_device_id *id)
{
	...
	als_data->pdata =  kmalloc(sizeof(*als_data->pdata), GFP_KERNEL);;
	if (NULL == als_data->pdata) {
		ERR("STKALS -- No enough memory\n");
		return -ENOMEM;
	}
	memcpy(als_data->pdata, client->dev.platform_data, sizeof(*als_data->pdata));
	...
	printk("als_data->pdata->gpio_int = %d, group =%d, pin = %d\n", 
			als_data->pdata->gpio_int,
			als_data->pdata->gpio_int / 32,
			als_data->pdata->gpio_int % 32);

	if (gpio_request(als_data->pdata->gpio_int, "EINT"))
	{
	...
	gpio_direction_input(als_data->pdata->gpio_int);

	client->irq = gpio_to_irq(als_data->pdata->gpio_int);
	if (client->irq < 0)
	{
		ERR("stk als gpio_to_irq() failed\n");
		err = -1;
		gpio_free(als_data->pdata->gpio_int);
		mutex_destroy(&stkals_io_lock);
		kfree(als_data);
		pStkAlsData = NULL;
		return err;
	...
	err = request_irq(client->irq, stk_oss_irq_handler, IRQF_TRIGGER_FALLING, DEVICE_NAME, als_data);
	if (err < 0)
	{
...
#if ADDITIONAL_GPIO_CFG // Additional GPIO CFG
	gpio_free(pStkAlsData->pdata->gpio_int);
#endif	//#if ADDITIONAL_GPIO_CFG
	mutex_destroy(&stkals_io_lock);
