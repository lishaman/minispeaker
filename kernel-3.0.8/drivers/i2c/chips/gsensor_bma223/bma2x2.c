...
static void bma2x2_set_enable(struct device *dev, int enable)
{
	...
	if (enable) {
		if (pre_enable == 0) {
			if (!IS_ERR(bma2x2->power))
				regulator_enable(bma2x2->power);
...
#endif
			if (!IS_ERR(bma2x2->power))
				regulator_disable(bma2x2->power);
		}
	}
		...
		const struct i2c_device_id *id)
{
	...
	data->power = regulator_get(NULL, "vcc_gsensor");
	if (IS_ERR(data->power)) {
		dev_err(&client->dev, "BMA2x2 get regulator failed");
	} else {
		regulator_enable(data->power);
	}
	...
	gpio_request(client->irq, "bma2x2_irq");
...
void bma2x2_shutdown(struct i2c_client *client)
{
	...
	mutex_unlock(&data->enable_mutex);
	if (!IS_ERR(data->power))
		regulator_force_disable(data->power);
}
