...
#if 0
static void cim_enable_with_dma(struct jz_camera_dev *pcdev)
{
	unsigned int regval = 0;
...
static int jz_camera_set_tlb_base(struct soc_camera_host *ici, unsigned int *tlb_base)
{
	unsigned int regval;
	struct jz_camera_dev *pcdev = ici->priv;

	pcdev->is_tlb_enabled = 1;
	/* bit[0:1] must be 0.double word aligned */
	if(*tlb_base & 0x3) {
...
static void jz_camera_init_videobuf(struct videobuf_queue *q, struct soc_camera_device *icd) {
	...
	videobuf_queue_dma_contig_init(q, &jz_videobuf_ops, icd->dev.parent,
			&pcdev->lock, V4L2_BUF_TYPE_VIDEO_CAPTURE, V4L2_FIELD_NONE,
			sizeof(struct jz_buffer), icd, &icd->video_lock);
}
...
static int jz_camera_set_crop(struct soc_camera_device *icd, struct v4l2_crop *a) {
	struct v4l2_subdev *sd = soc_camera_to_subdev(icd);

	return v4l2_subdev_call(sd, video, s_crop, a);
}

static int jz_camera_set_bus_param(struct soc_camera_device *icd, __u32 pixfmt) {
	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
	struct jz_camera_dev *pcdev = ici->priv;
	...
	common_flags = soc_camera_bus_param_compatible(camera_flags, CIM_BUS_FLAGS);
	if (!common_flags)
		return -EINVAL;
	...
	if ((common_flags & SOCAM_VSYNC_ACTIVE_HIGH) &&
		...
		if (!pcdev->pdata || pcdev->pdata->flags & JZ_CAMERA_VSYNC_HIGH)
			common_flags &= (~SOCAM_VSYNC_ACTIVE_LOW);
		else
	...
	if ((common_flags & SOCAM_PCLK_SAMPLE_RISING) &&
		...
		if (!pcdev->pdata || pcdev->pdata->flags & JZ_CAMERA_PCLK_RISING)
			common_flags &= (~SOCAM_PCLK_SAMPLE_FALLING);
		else
	...
	if ((common_flags & SOCAM_DATA_ACTIVE_HIGH) &&
		...
		if (!pcdev->pdata || pcdev->pdata->flags & JZ_CAMERA_DATA_HIGH)
			common_flags &= (~SOCAM_DATA_ACTIVE_LOW);
		else
...
static int jz_camera_add_device(struct soc_camera_device *icd) {
	...
	dprintk(6, "jz Camera driver attached to camera %d\n",
			icd->devnum);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("WuZhe <zhe.wu@ingenic.com>");
