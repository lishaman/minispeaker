...
static int ovisp_vb2_mmap(void *buf_priv, struct vm_area_struct *vma)
{
	...
	if (!buf) {
		ISP_PRINT(ISP_ERROR,KERN_ERR "No buffer to map\n");
		return -EINVAL;
	}
					...
					unsigned long size, int write)
{
	...
	if (!buf)
		return ERR_PTR(-ENOMEM);
#if 0// we need no physical address when we use tlb.
	ret = vb2_get_contig_userptr(vaddr, size, &vma, &paddr);
	if (ret) {
		ISP_PRINT(ISP_ERROR,KERN_ERR "Failed acquiring VMA for vaddr 0x%08lx\n",
				vaddr);
		kfree(buf);
		return ERR_PTR(ret);
	}
...
#endif
	memset(conf->vaddr, 0x2f, conf->size);
	if (!conf->vaddr) {
