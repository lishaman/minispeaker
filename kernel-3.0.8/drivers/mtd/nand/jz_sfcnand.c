...
int sfc_nand_plat_resource_init(struct jz_sfc_nand *flash,struct platform_device *pdev)
{
	...
	flash->clk = clk_get(&pdev->dev, "cgu_sfc");//change cgu_ssi to cgu_sfc
	if (IS_ERR(flash->clk)) {
...
struct jz_spi_support *jz_sfc_nand_probe(struct jz_sfc_nand *flash)
{
	...
	struct jz_spi_support *params=NULL;
...
static void dumpdata(u_char *buf, uint len)
{
	uint i = 1;
...
//	if ((ops_addr >= 0x900000) & (ops_addr < (0x900000 + 3 *128 * 1024)))
//	printk("jz_sfc_nandflash_read: ops_addr = 0x%x, len = %d\n",ops_addr,len);//read rootfs
...
//	if ((addr >= 0x900000) & (addr < (0x900000 + 3 *128 * 1024)))
	if(addr == 0x1921000)
		dumpdata(buf,64);
...
static int __init jz_sfc_probe(struct platform_device *pdev)
{
	...
	int num_partitions;
	struct jz_spi_nand_platform_data *param=NULL;
	int nand_magic= 0;
