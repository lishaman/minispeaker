...
static int jz_nand_probe(struct platform_device *pdev)
{
	...
	if (gpio_is_valid(nand->rb_gpio)) {
		ret = gpio_request_one(nand->rb_gpio, GPIOF_DIR_IN, "nand-bank0-rb");
		if (ret)
			goto err_request_rb_gpio;
	} else
		nand->rb_gpio = -1;

	nand->wp_gpio = pdata->wp_gpio;
	if (gpio_is_valid(nand->wp_gpio)) {
		ret = gpio_request_one(nand->wp_gpio, GPIOF_INIT_LOW|GPIOF_DIR_OUT,
				"nand-band0-wp");
		if (ret)
			goto err_request_wp_gpio;
	} else
		nand->wp_gpio = -1;
	...
	if (nand_scan_ident(&nand->mtd, 1, nand_table_ids)) {
		ret = -ENXIO;
...
err_platform_register:
	if (gpio_is_valid(nand->wp_gpio))
		gpio_free(nand->wp_gpio);
err_request_wp_gpio:
	gpio_free(nand->rb_gpio);
err_request_rb_gpio:
	return ret;
}
