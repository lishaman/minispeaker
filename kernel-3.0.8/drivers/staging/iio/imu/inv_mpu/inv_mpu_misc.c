...
*/
static int inv_check_accel_self_test(struct inv_mpu_iio_s *st,
		...
		int *gyro_result, int *accl_result)
{
	...
	if (INV_MPU6500 == st->chip_type) {
		...
		result = inv_i2c_single_write(st, 0x29, 2);	//Accel LPF
		if (result)
			return result;
	}
	...
	printk("Accel offset data ST=%d, AX=%d, AY=%d, AZ=%d\n",
				self_test_flag, accl_result[0], 
				accl_result[1], accl_result[2]);

	printk("Gyro offset data ST=%d, GX=%d, GY=%d, GZ=%d\n",
...
void inv_recover_setting(struct inv_mpu_iio_s *st)
{
	...
	if (INV_MPU6500 == st->chip_type) {
		inv_i2c_single_write(st, 0x29, 0);	//Recover Accel LPF
	}
...
int inv_set_tap_axes_dmp(struct inv_mpu_iio_s *st,
				u32 axes)
{
...
int inv_set_multiple_tap_time_dmp(struct inv_mpu_iio_s *st,
					u32 time)
{
...
int inv_set_shake_reject_thresh_dmp(struct inv_mpu_iio_s *st,
						int thresh)
...
int inv_set_shake_reject_time_dmp(struct inv_mpu_iio_s *st,
						u32 time)
{
...
int inv_set_shake_reject_timeout_dmp(struct inv_mpu_iio_s *st,
						u32 time)
{
...
int inv_enable_tap_dmp(struct inv_mpu_iio_s *st, bool on)
{
	...
	result = inv_set_tap_axes_dmp(st, st->tap.axis);
	if (result)
		return result;
	...
	result = inv_set_multiple_tap_time_dmp(st, st->tap.multi_tap_time);
	if (result)
		return result;
	...
	result = inv_set_shake_reject_thresh_dmp(st, st->tap.shake_reject_thresh);
	if (result)
		return result;

	result = inv_set_shake_reject_time_dmp(st, st->tap.shake_reject_time);
	if (result)
		return result;
