...
static void get_slop_cut(struct jz_battery *jz_battery)
{
	unsigned int tmp = 0;
	//jz_efuse_read(4,0,&tmp);
	jz_battery->slop = 2 *( (tmp>>24) & 0xff ) + 2895;
	jz_battery->cut = 1000 * ( (tmp>>16)&0xff ) + 90000;
}
...
static int jz_battery_adjust_voltage(struct jz_battery *battery)
{
	...
	if ((battery->slop == 2895) && (battery->cut == 90000)) {
		real_voltage = final_value * 1200 / 4096;
		real_voltage *= 4;
		pr_debug("Battery driver:The voltage is %dmV\n",real_voltage);
	} else {
		real_voltage = (final_value * battery->slop + battery->cut) / 10000;
		real_voltage *= 4;
...
MODULE_ALIAS("platform:jz-battery");
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Sun Jiwei<jwsun@ingenic.cn>");
MODULE_DESCRIPTION("JZ SoC battery driver");
