...
wlan_ops_sta_process_rx_packet(IN t_void * adapter, IN pmlan_buffer pmbuf)
{
     ...
     */
    if ((!IS_11N_ENABLED(priv) && !(prx_pd->flags & RXPD_FLAG_PKT_DIRECT_LINK))
    ...
    if (queuing_ra_based(priv) || (prx_pd->flags & RXPD_FLAG_PKT_DIRECT_LINK)) {
    ...
    if ((ret = mlan_11n_rxreorder_pkt(priv, prx_pd->seq_num,
        ...
        if ((ret =
             pmadapter->callbacks.moal_recv_complete(pmadapter->pmoal_handle,
                                                     ...
                                                     ret)))
            PRINTM(MERROR, "STA Rx Error: moal_recv_complete returned error\n");
    }
