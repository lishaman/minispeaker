    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        if (!(misc->param.heart_beat.h2d_timer)) {
            ...
            pcb->moal_start_timer(pmadapter->pmoal_handle,
                                  pmadapter->hb_host_timer, MFALSE,
        ...
        if (!(misc->param.heart_beat.d2h_timer)) {
            ...
            pcb->moal_start_timer(pmadapter->pmoal_handle,
                                  pmadapter->hb_device_timer, MFALSE,
 ...
 */
mlan_status
wlan_misc_ioctl_fw_auto_reconnect(IN pmlan_adapter pmadapter,
                                  IN pmlan_ioctl_req pioctl_req)
{
    mlan_private *pmpriv = pmadapter->priv[pioctl_req->bss_index];
    mlan_status ret = MLAN_STATUS_SUCCESS;
...
wlan_misc_cfg_ioctl(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    case MLAN_OID_MISC_AUTO_ASSOC:
        status = wlan_misc_ioctl_fw_auto_reconnect(pmadapter, pioctl_req);
        break;
