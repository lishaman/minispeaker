                           ...
                           mlan_ioctl_req * pioctl_buf)
{
    ...
    case HostCmd_CMD_TDLS_CONFIG:
        ...
        case WLAN_TDLS_SETUP_REQ:
            ...
            tdls_all_cfg =
                (tdls_all_config *) ptdls_config_data->tdls_info.tdls_data;
            PRINTM(MERROR, "TDLS Setup Failed, error %d\n",
                   wlan_le16_to_cpu(tdls_all_cfg->u.tdls_cmd_resp.reason_code));
            break;
        case WLAN_TDLS_INIT_CHAN_SWITCH:
            tdls_all_cfg =
                (tdls_all_config *) ptdls_config_data->tdls_info.tdls_data;
            PRINTM(MERROR,
                   "TDLS init channel switch failed," MACSTR ": reason=%d\n",
                   MAC2STR(tdls_all_cfg->u.tdls_cmd_resp.peer_mac_addr),
                   wlan_le16_to_cpu(tdls_all_cfg->u.tdls_cmd_resp.reason_code));
            break;
        }
        break;
...
wlan_set_tdls_powermode(IN pmlan_private pmpriv, t_u8 powermode)
{
    ...
    if (powermode) {
        ...
        if (!pmpriv->adapter->sleep_period.period)
            pmpriv->adapter->sleep_period.period = DEFAULT_TDLS_SLEEP_PERIOD;
    } else {
                     ...
                     OUT mlan_ioctl_req * pioctl_buf)
{
    ...
    case WLAN_TDLS_DISCOVERY_REQ:
        ...
        tdls_all_cfg->u.tdls_discovery_resp.payload_len =
            wlan_le16_to_cpu(tdls_all_cfg->u.tdls_discovery_resp.payload_len);
        tdls_all_cfg->u.tdls_discovery_resp.cap_info =
            wlan_le16_to_cpu(tdls_all_cfg->u.tdls_discovery_resp.cap_info);
        memcpy(pmpriv->adapter, &misc->param.tdls_config,
               &ptdls_config_data->tdls_info,
               MIN(sizeof(mlan_ds_misc_tdls_config), (resp->size - S_DS_GEN)));
        PRINTM(MCMND, "TDLS_DISCOVERY_REQ: " MACSTR "\n",
               MAC2STR(tdls_all_cfg->u.tdls_discovery_resp.peer_mac_addr));
        break;
    ...
    case WLAN_TDLS_INIT_CHAN_SWITCH:
        tdls_all_cfg =
            (tdls_all_config *) ptdls_config_data->tdls_info.tdls_data;
        PRINTM(MCMND,
               "TDLS_INIT_CHANNEL_SWITCH: " MACSTR " chan=%d periodicity=%d\n",
    ...
    case WLAN_TDLS_LINK_STATUS:
        ...
        tdls_all_cfg->u.tdls_link_status_resp.payload_len =
            wlan_le16_to_cpu(tdls_all_cfg->u.tdls_link_status_resp.payload_len);
        link_ptr = tdls_all_cfg->u.tdls_link_status_resp.link_stats;
        for (i = 0; i < tdls_all_cfg->u.tdls_link_status_resp.active_links; i++) {
            ...
            link_ptr->data_nf_last = wlan_le16_to_cpu(link_ptr->data_nf_last);
            link_ptr->data_rssi_avg = wlan_le16_to_cpu(link_ptr->data_rssi_avg);
            link_ptr->data_nf_avg = wlan_le16_to_cpu(link_ptr->data_nf_avg);
            ...
            link_ptr =
                (tdls_each_link_status *) (((t_u8 *) link_ptr) + link_length);
        }
        memcpy(pmpriv->adapter, &misc->param.tdls_config,
               &ptdls_config_data->tdls_info,
               MIN(sizeof(mlan_ds_misc_tdls_config), (resp->size - S_DS_GEN)));
    ...
    case WLAN_TDLS_STOP_CHAN_SWITCH:
        ...
        PRINTM(MCMND, "TDLS_STOP_CHANNEL_SWITCH: " MACSTR "\n",
               MAC2STR(tdls_all_cfg->u.tdls_stop_chan_switch.peer_mac_addr));
        break;
                    ...
                    IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        misc->param.heart_beat.reconnect_delay =
            wlan_le16_to_cpu(cmd_heart_beat->reconnect_delay);
    }
 ...
 */
static mlan_status
wlan_ret_fw_auto_reconnect(IN pmlan_private pmpriv,
                           IN HostCmd_DS_COMMAND * resp,
                           IN mlan_ioctl_req * pioctl_buf)
{
    HostCmd_DS_FW_AUTO_RECONNECT *fw_auto_reconnect =
        (HostCmd_DS_FW_AUTO_RECONNECT *) & resp->params.fw_auto_reconnect_cmd;
    mlan_ds_misc_cfg *misc = MNULL;
                                         ...
                                         IN mlan_ioctl_req * pioctl_buf)
{
    ...
    tlv_buf_len =
        resp->size - (sizeof(HostCmd_DS_PACKET_AGGR_OVER_HOST_INTERFACE) +
                      S_DS_GEN);
        ...
        while (tlv_buf_len > 0) {
            ...
            case MRVL_USB_AGGR_PARAM_TLV_ID:
                ...
                usb_aggr_param_tlv->header.len =
                    wlan_le16_to_cpu(usb_aggr_param_tlv->header.len);
                usb_aggr_param_tlv->enable =
                    wlan_le16_to_cpu(usb_aggr_param_tlv->enable);
                if (pioctl_buf->action == MLAN_ACT_SET) {
                    /* Update the Tx aggregation values in MLAN */
                    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
                        pmadapter->usb_tx_aggr[i].aggr_ctrl.enable =
                            usb_aggr_ctrl->tx_aggr_ctrl.enable;
                        if (pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_mode !=
                            usb_aggr_ctrl->tx_aggr_ctrl.aggr_mode) {
                            pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_mode =
                                usb_aggr_ctrl->tx_aggr_ctrl.aggr_mode;
                            changed = 1;
                        }
                        if (pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_align !=
                            usb_aggr_ctrl->tx_aggr_ctrl.aggr_align) {
                            pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_align =
                                usb_aggr_ctrl->tx_aggr_ctrl.aggr_align;
                            changed = 1;
                        }
                        if (pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_max !=
                            usb_aggr_ctrl->tx_aggr_ctrl.aggr_max) {
                            pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_max =
                                usb_aggr_ctrl->tx_aggr_ctrl.aggr_max;
                            changed = 1;
                        }
                    ...
                    if (usb_aggr_param_tlv->enable & MBIT(1))
                        usb_aggr_ctrl->tx_aggr_ctrl.enable = MTRUE;
                    else
                        usb_aggr_ctrl->tx_aggr_ctrl.enable = MFALSE;
                    usb_aggr_ctrl->tx_aggr_ctrl.aggr_align =
                        wlan_le16_to_cpu(usb_aggr_param_tlv->tx_aggr_align);
                    usb_aggr_ctrl->tx_aggr_ctrl.aggr_mode =
                        pmadapter->usb_tx_aggr[0].aggr_ctrl.aggr_mode;
                    usb_aggr_ctrl->tx_aggr_ctrl.aggr_max =
                        pmadapter->usb_tx_aggr[0].aggr_ctrl.aggr_max;
                    usb_aggr_ctrl->tx_aggr_ctrl.aggr_tmo =
                        pmadapter->usb_tx_aggr[0].aggr_ctrl.aggr_tmo;
                }
                if (changed)
                    wlan_reset_usb_tx_aggr(pmadapter);

                if (pioctl_buf->action == MLAN_ACT_SET) {
                    /* Update the Rx deaggregation values in MLAN */
                    pmadapter->usb_rx_deaggr.aggr_ctrl.enable =
                        usb_aggr_ctrl->rx_deaggr_ctrl.enable;
                    if (pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_mode !=
                        usb_aggr_ctrl->rx_deaggr_ctrl.aggr_mode)
                        pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_mode =
                            usb_aggr_ctrl->rx_deaggr_ctrl.aggr_mode;
                    if (pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_align !=
                        usb_aggr_ctrl->rx_deaggr_ctrl.aggr_align)
                        pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_align =
                            usb_aggr_ctrl->rx_deaggr_ctrl.aggr_align;
                    if (pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_max !=
                        usb_aggr_ctrl->rx_deaggr_ctrl.aggr_max)
                        pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_max =
                            usb_aggr_ctrl->rx_deaggr_ctrl.aggr_max;
                    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_tmo =
                        usb_aggr_ctrl->rx_deaggr_ctrl.aggr_tmo;
                } else {
                    if (usb_aggr_param_tlv->enable & MBIT(0))
                        usb_aggr_ctrl->rx_deaggr_ctrl.enable = MTRUE;
                    else
                        usb_aggr_ctrl->rx_deaggr_ctrl.enable = MFALSE;
                    usb_aggr_ctrl->rx_deaggr_ctrl.aggr_mode =
                        wlan_le16_to_cpu(usb_aggr_param_tlv->rx_aggr_mode);
                    usb_aggr_ctrl->rx_deaggr_ctrl.aggr_align =
                        wlan_le16_to_cpu(usb_aggr_param_tlv->rx_aggr_align);
                    usb_aggr_ctrl->rx_deaggr_ctrl.aggr_max =
                        wlan_le16_to_cpu(usb_aggr_param_tlv->rx_aggr_max);
                    usb_aggr_ctrl->rx_deaggr_ctrl.aggr_tmo =
                        wlan_le16_to_cpu(usb_aggr_param_tlv->rx_aggr_tmo);
                }
                ptlv_buffer +=
                    usb_aggr_param_tlv->header.len +
                    sizeof(MrvlIEtypesHeader_t);
                             ...
                             IN t_void * pcmd_buf, IN t_void * pioctl)
{
    ...
    case HostCmd_CMD_PACKET_AGGR_OVER_HOST_INTERFACE:
        ret =
            wlan_ret_packet_aggr_over_host_interface(pmpriv, resp, pioctl_buf);
        break;
