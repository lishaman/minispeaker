    ...
    for (region_idx = 0;
        ...
        for (next_chan = 0;
            ...
            if (scan_type == MLAN_SCAN_TYPE_PASSIVE) {
                ...
                pscan_chan_list[chan_idx].chan_scan_mode.hidden_ssid_report =
                    MTRUE;
            } else {
        ...
        for (chan_idx = 0;
            ...
            if (scan_type == MLAN_SCAN_TYPE_PASSIVE) {
                (pscan_chan_list + chan_idx)->chan_scan_mode.passive_scan =
                    MTRUE;
                (pscan_chan_list +
                 chan_idx)->chan_scan_mode.hidden_ssid_report = MTRUE;
            } else {
                (pscan_chan_list + chan_idx)->chan_scan_mode.passive_scan =
                    MFALSE;
            }
                                      ...
                                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    for (bss_idx = 0; bss_idx < pmadapter->num_in_scan_table; bss_idx++) {
        scan_reqd = MFALSE;
        if (!memcmp(pmadapter, pmadapter->pscan_table[bss_idx].ssid.ssid,
                    null_ssid, pmadapter->pscan_table[bss_idx].ssid.ssid_len)) {
            if (pmadapter->scan_type == MLAN_SCAN_TYPE_PASSIVE)
                scan_reqd = MTRUE;
            else {
            ...
            if (scan_reqd) {
                chan_listed = MFALSE;
                for (i = 0; i < id; i++) {
                    if ((user_scan_cfg->chan_list[i].chan_number ==
                         pmadapter->pscan_table[bss_idx].channel)
                        && (user_scan_cfg->chan_list[i].radio_type & pmadapter->
                            pscan_table[bss_idx].bss_band)) {
                        chan_listed = MTRUE;
                        break;
                    }
                }
    ...
    if (id) {
        ...
        memcpy(pmpriv->adapter, user_scan_cfg->ssid_list[0].ssid,
               pscan_req->scan_ssid.ssid, pscan_req->scan_ssid.ssid_len);
                     ...
                     IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
    ...
    if (!util_peek_list
        (pmadapter->pmoal_handle, &pmadapter->scan_pending_q,
         pcb->moal_spin_lock, pcb->moal_spin_unlock)) {
        if (!wlan_11d_is_enabled(pmpriv)) {
            if (((mlan_ds_scan *) pioctl_req->pbuf)->sub_command ==
                MLAN_OID_SCAN_SPECIFIC_SSID) {
                if (wlan_active_scan_req_for_passive_chan(pmpriv, pioctl_req)) {
                    goto done;
                }
            }
        }
