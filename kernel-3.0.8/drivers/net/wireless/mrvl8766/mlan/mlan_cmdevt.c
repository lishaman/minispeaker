                   ...
                   IN t_void * pioctl_buf, IN t_void * pdata_buf)
{
    ...
    if (!(pcmd_node->cmdbuf = wlan_alloc_mlan_buffer(pmadapter,
...
wlan_dnld_cmd_to_fw(IN mlan_private * pmpriv, IN cmd_ctrl_node * pcmd_node)
{
    ...
    if (pcmd_node->cmdbuf->data_offset < MLAN_TYPE_LEN) {
        PRINTM(MERROR, "DNLD_CMD: data_offset is too small=%d\n",
               pcmd_node->cmdbuf->data_offset);
        if (pioctl_buf)
            pioctl_buf->status_code = MLAN_ERROR_CMD_DNLD_FAIL;
        wlan_insert_cmd_to_free_q(pmadapter, pcmd_node);
    ...
    memcpy(pmadapter, (t_u8 *) pcmd - MLAN_TYPE_LEN, (t_u8 *) & tmp,
           MLAN_TYPE_LEN);
    pcmd_node->cmdbuf->data_offset -= MLAN_TYPE_LEN;
    ...
    ret = pcb->moal_write_data_async(pmadapter->pmoal_handle,
                                     pcmd_node->cmdbuf, MLAN_USB_EP_CMD_EVENT);
...
wlan_dnld_sleep_confirm_cmd(mlan_adapter * pmadapter)
{
    ...
    memcpy(pmadapter, pmbuf, pmadapter->psleep_cfm, sizeof(mlan_buffer));
    memcpy(pmadapter, pmbuf->pbuf + pmbuf->data_offset,
           pmadapter->psleep_cfm->pbuf + pmadapter->psleep_cfm->data_offset,
...
wlan_process_event(pmlan_adapter pmadapter)
{
    ...
    if (MTRUE && (eventcause != EVENT_PS_AWAKE)
        ) {
                       ...
                       IN mlan_ioctl_req * pioctl_buf)
{
    ...
    PRINTM(MCMND,
           ...
           phs_cfg->params.hs_config.conditions, phs_cfg->params.hs_config.gpio,
