                     ...
                     IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    HostCmd_DS_TDLS_CONFIG *ptdls_config_data = &(cmd->params.tdls_config_data);
    t_u8 zero_mac[] = { 0, 0, 0, 0, 0, 0 };
    ...
    case WLAN_TDLS_SET_INFO:
        ...
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
               (t_u8 *) & tdls_all_cfg->u.tdls_set.cap_info, sizeof(t_u16));
        memcpy(pmpriv->adapter, (t_u8 *) ptdls_config_data->tdls_info.tdls_data
               + sizeof(t_u16),
    ...
    case WLAN_TDLS_DISCOVERY_REQ:
        ...
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
               tdls_all_cfg->u.tdls_discovery.peer_mac_addr, travel_len);
        break;

    case WLAN_TDLS_SETUP_REQ:
        travel_len = sizeof(tdls_all_cfg->u.tdls_setup);
        tdls_all_cfg->u.tdls_setup.setup_timeout =
            wlan_cpu_to_le32(tdls_all_cfg->u.tdls_setup.setup_timeout);
        tdls_all_cfg->u.tdls_setup.key_lifetime =
            wlan_cpu_to_le32(tdls_all_cfg->u.tdls_setup.key_lifetime);
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
               &tdls_all_cfg->u.tdls_setup, travel_len);
        break;
    ...
    case WLAN_TDLS_TEAR_DOWN_REQ:
        ...
        tdls_all_cfg->u.tdls_tear_down.reason_code =
            wlan_cpu_to_le16(tdls_all_cfg->u.tdls_tear_down.reason_code);
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
    ...
    case WLAN_TDLS_STOP_CHAN_SWITCH:
        travel_len = MLAN_MAC_ADDR_LENGTH;
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
               tdls_all_cfg->u.tdls_stop_chan_switch.peer_mac_addr, travel_len);
        break;
    case WLAN_TDLS_INIT_CHAN_SWITCH:
        ...
        tdls_all_cfg->u.tdls_chan_switch.switch_time =
            wlan_cpu_to_le16(tdls_all_cfg->u.tdls_chan_switch.switch_time);
        tdls_all_cfg->u.tdls_chan_switch.switch_timeout =
            wlan_cpu_to_le16(tdls_all_cfg->u.tdls_chan_switch.switch_timeout);
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
    ...
    case WLAN_TDLS_POWER_MODE:
        ...
        tdls_all_cfg->u.tdls_power_mode.power_mode =
            wlan_cpu_to_le16(tdls_all_cfg->u.tdls_power_mode.power_mode);
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
    ...
    case WLAN_TDLS_LINK_STATUS:
        ...
        if (memcmp
            (pmpriv->adapter,
             tdls_all_cfg->u.tdls_link_status_req.peer_mac_addr, zero_mac,
             sizeof(zero_mac))) {
            travel_len = sizeof(tdls_all_cfg->u.tdls_link_status_req);
            memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
    ...
    case WLAN_TDLS_DEBUG_CS_RET_IM:
        ...
        tdls_all_cfg->u.tdls_debug_data.debug_data =
            wlan_cpu_to_le16(tdls_all_cfg->u.tdls_debug_data.debug_data);
        memcpy(pmpriv->adapter, ptdls_config_data->tdls_info.tdls_data,
 ...
 */
static mlan_status
wlan_cmd_fw_auto_reconnect(IN pmlan_private pmpriv,
                           IN HostCmd_DS_COMMAND * cmd,
                           IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    HostCmd_DS_FW_AUTO_RECONNECT *fw_auto_reconnect =
        &cmd->params.fw_auto_reconnect_cmd;
    mlan_ds_fw_reconnect *fw_auto_reconn = (mlan_ds_fw_reconnect *) pdata_buf;

    ENTER();

    cmd->command = wlan_cpu_to_le16(HostCmd_CMD_FW_AUTO_RECONNECT);
    cmd->size =
        wlan_cpu_to_le16((sizeof(HostCmd_DS_FW_AUTO_RECONNECT)) + S_DS_GEN);
                                         ...
                                         IN t_void * pdata_buf)
{
    ...
    cmd->size =
        wlan_cpu_to_le16(sizeof(HostCmd_DS_PACKET_AGGR_OVER_HOST_INTERFACE) +
    ...
    if (cmd_action == HostCmd_ACT_GEN_SET) {
        ...
        usb_aggr_param_tlv->tx_aggr_align =
            wlan_cpu_to_le16(usb_aggr_ctrl->tx_aggr_ctrl.aggr_align);
        if (usb_aggr_ctrl->rx_deaggr_ctrl.enable)
            usb_aggr_param_tlv->enable |= MBIT(0);
        usb_aggr_param_tlv->rx_aggr_mode =
            wlan_cpu_to_le16(usb_aggr_ctrl->rx_deaggr_ctrl.aggr_mode);
        usb_aggr_param_tlv->rx_aggr_align =
            wlan_cpu_to_le16(usb_aggr_ctrl->rx_deaggr_ctrl.aggr_align);
        usb_aggr_param_tlv->rx_aggr_max =
            wlan_cpu_to_le16(usb_aggr_ctrl->rx_deaggr_ctrl.aggr_max);
        usb_aggr_param_tlv->rx_aggr_tmo =
            wlan_cpu_to_le16(usb_aggr_ctrl->rx_deaggr_ctrl.aggr_tmo);
                         ...
                         IN t_void * pdata_buf, IN t_void * pcmd_buf)
{
    ...
    case HostCmd_CMD_TDLS_CONFIG:
        ret = wlan_cmd_tdls_config(pmpriv, cmd_ptr, cmd_action, pdata_buf);
        break;
    ...
    case HostCmd_CMD_FW_AUTO_RECONNECT:
        ret =
            wlan_cmd_fw_auto_reconnect(pmpriv, cmd_ptr, cmd_action, pdata_buf);
        break;
    ...
    case HostCmd_CMD_PACKET_AGGR_OVER_HOST_INTERFACE:
        ret =
            wlan_cmd_packet_aggr_over_host_interface(pmpriv, cmd_ptr,
                                                     cmd_action, pdata_buf);
        break;
    default:
