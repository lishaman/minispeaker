    ...
    if ((pmbuf->buf_type == MLAN_BUF_TYPE_DATA) ||
        ...
        if (pmbuf->flags & MLAN_BUF_FLAG_MOAL_TX_BUF) {
            ...
            if ((pmbuf->flags & MLAN_BUF_FLAG_USB_TX_AGGR) &&
                ...
                /* don't complete pmbuf(=aggr), it has no skb */
                /* it will be handled after this loop */
                pmbuf_curr = pmbuf->pnext;
                for (i = 0; i < use_count; i++) {
                    pmbuf_next = pmbuf_curr->pnext;
                    if (pmbuf_curr->flags & MLAN_BUF_FLAG_MOAL_TX_BUF)
                        pcb->moal_send_packet_complete(pmadapter->pmoal_handle,
                                                       pmbuf_curr, status);
                    else
                        wlan_free_mlan_buffer(pmadapter, pmbuf_curr);
