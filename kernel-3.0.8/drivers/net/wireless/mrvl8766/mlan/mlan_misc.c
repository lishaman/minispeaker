                         ...
                         IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        pmadapter->dbg.num_card_to_host_failure =
            info->param.debug_info.num_card_to_host_failure;
...
wlan_host_heart_beat(void *function_context)
{
    ...
    pcb->moal_start_timer(pmadapter->pmoal_handle, pmadapter->hb_host_timer,
...
wlan_device_heart_beat(void *function_context)
{
    ...
    if ((pmadapter->is_hb_ignored == MFALSE)
        ...
        if (pmadapter->hb_dev_counter > 2) {
            ...
            PRINTM(MFATAL, "ERROR: No heart beat from device!\n");
        }
    }
    pcb->moal_start_timer(pmadapter->pmoal_handle, pmadapter->hb_device_timer,
                            ...
                            IN pmlan_ioctl_req pioctl_req)
{
    ...
    ret = wlan_prepare_cmd(pmpriv,
                           ...
                           HostCmd_ACT_GEN_SET,
                           0, (t_void *) pioctl_req, &misc->param.tdls_config);
 ...
 */
mlan_status
wlan_misc_ioctl_usb_aggr_ctrl(IN pmlan_adapter pmadapter,
                              IN pmlan_ioctl_req pioctl_req)
{
    mlan_status ret = MLAN_STATUS_SUCCESS;
    mlan_ds_misc_cfg *misc = MNULL;
    ...
    if (pmadapter->fw_usb_aggr == MFALSE) {
    ...
    ret = wlan_prepare_cmd(pmpriv,
                           ...
                           cmd_action,
                           0,
                           (t_void *) pioctl_req, &misc->param.usb_aggr_params);
