...
wlan_init_adapter(pmlan_adapter pmadapter)
{
    ...
    t_s32 i = 0;
    const t_u32 usb_tx_ports[] = { MLAN_USB_EP_DATA };
    ENTER();
    ...
    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
        pmadapter->usb_tx_aggr[i].aggr_ctrl.enable = MFALSE;
        pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_mode = MLAN_USB_AGGR_MODE_NUM;
        pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_align =
            MAX(pmadapter->max_tx_buf_size, MLAN_USB_TX_AGGR_ALIGN);
        pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_max = MLAN_USB_TX_MAX_AGGR_NUM;
        pmadapter->usb_tx_aggr[i].aggr_ctrl.aggr_tmo =
            MLAN_USB_TX_AGGR_TIMEOUT_MSEC * 1000;

        pmadapter->usb_tx_aggr[i].pmbuf_aggr = MNULL;
        pmadapter->usb_tx_aggr[i].hold_timeout_msec =
            MLAN_USB_TX_AGGR_TIMEOUT_MSEC;
        pmadapter->usb_tx_aggr[i].port = usb_tx_ports[i];
        pmadapter->usb_tx_aggr[i].phandle = (t_void *) pmadapter;
    }

    pmadapter->usb_rx_deaggr.aggr_ctrl.enable = MFALSE;
    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_mode = MLAN_USB_AGGR_MODE_NUM;
    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_align = MLAN_USB_RX_ALIGN_SIZE;
    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_max = MLAN_USB_RX_MAX_AGGR_NUM;
    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_tmo =
        MLAN_USB_RX_DEAGGR_TIMEOUT_USEC;

    pmadapter->fw_usb_aggr = MTRUE;
    ...
    if (pmadapter->psleep_cfm) {
        ...
        sleep_cfm_buf->hdr = wlan_cpu_to_le32(MLAN_USB_TYPE_CMD);
...
wlan_free_lock_list(IN pmlan_adapter pmadapter)
{
    ...
    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
        if (pmadapter->usb_tx_aggr[i].paggr_lock)
            pcb->moal_free_lock(pmadapter->pmoal_handle,
                                pmadapter->usb_tx_aggr[i].paggr_lock);
    }
...
wlan_init_timer(IN pmlan_adapter pmadapter)
{
    ...
    if (pcb->
        moal_init_timer(pmadapter->pmoal_handle, &pmadapter->hb_device_timer,
                        wlan_device_heart_beat, pmadapter)
        != MLAN_STATUS_SUCCESS) {
        ret = MLAN_STATUS_FAILURE;
        goto error;
    }
    ...
    if (pcb->
        moal_init_timer(pmadapter->pmoal_handle, &pmadapter->hb_host_timer,
                        wlan_host_heart_beat, pmadapter)
        != MLAN_STATUS_SUCCESS) {
        ret = MLAN_STATUS_FAILURE;
        goto error;
    }
    ...
    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
        if (pcb->
            moal_init_timer(pmadapter->pmoal_handle,
                            &pmadapter->usb_tx_aggr[i].paggr_hold_timer,
...
wlan_free_timer(IN pmlan_adapter pmadapter)
{
    ...
    if (pmadapter->hb_device_timer)
        pcb->moal_free_timer(pmadapter->pmoal_handle,
                             pmadapter->hb_device_timer);
    if (pmadapter->hb_host_timer)
        pcb->moal_free_timer(pmadapter->pmoal_handle, pmadapter->hb_host_timer);
    if (pmadapter->pmlan_cmd_timer)
        pcb->moal_free_timer(pmadapter->pmoal_handle,
                             pmadapter->pmlan_cmd_timer);
    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
        if (pmadapter->usb_tx_aggr[i].paggr_hold_timer)
            pcb->moal_free_timer(pmadapter->pmoal_handle,
                                 pmadapter->usb_tx_aggr[i].paggr_hold_timer);
    }
...
wlan_free_adapter(pmlan_adapter pmadapter)
{
    ...
    for (i = 0; i < MAX_USB_TX_PORT_NUM; i++) {
        if (pmadapter->usb_tx_aggr[i].aggr_hold_timer_is_set) {
            ...
            pcb->moal_stop_timer(pmadapter->pmoal_handle,
                                 pmadapter->usb_tx_aggr[i].paggr_hold_timer);
            pmadapter->usb_tx_aggr[i].aggr_hold_timer_is_set = MFALSE;
        }
    }
    if (pmadapter->hb_host_timer_is_set) {
        pcb->moal_stop_timer(pmadapter->pmoal_handle, pmadapter->hb_host_timer);
        pmadapter->hb_host_timer_is_set = MFALSE;
    }
