...
wlan_cleanup_tdls_txq(pmlan_private priv)
{
    ...
    while ((pmbuf =
            (pmlan_buffer) util_peek_list(pmadapter->pmoal_handle,
                                          ...
                                          MNULL))) {
        util_unlink_list(pmadapter->pmoal_handle, &priv->tdls_pending_txq,
...
wlan_send_processed_packet(pmlan_private priv, raListTbl * ptr, int ptrindex)
{
    ...
    if ((pmbuf = (pmlan_buffer) util_dequeue_list(pmadapter->pmoal_handle,
                                                  &ptr->buf_head,
                                                  MNULL, MNULL))) {
        if ((pusb_tx_aggr = wlan_get_usb_tx_aggr_params(pmadapter, priv->port))) {
            pmbuf_next =
                (pmlan_buffer) util_peek_list(pmadapter->pmoal_handle,
                                              &ptr->buf_head, MNULL, MNULL);
        }
        ...
        if (pusb_tx_aggr) {
            tx_param.next_pkt_len = ((pmbuf_next) ?
                                     pmbuf_next->data_len + sizeof(TxPD) : 0);
            ret =
                wlan_usb_host_to_card_aggr(pmadapter, pmbuf, &tx_param,
                                           pusb_tx_aggr);
        } else {
            pmadapter->data_sent = MTRUE;

            ret =
                pmadapter->callbacks.moal_write_data_async(pmadapter->
                                                           pmoal_handle, pmbuf,
                                                           priv->port);
        }
        switch (ret) {
...
wlan_update_non_tdls_ralist(mlan_private * priv, t_u8 * mac, t_u8 tx_pause)
{
    ...
    for (i = 0; i < MAX_NUM_TID; ++i) {
        ra_list = (raListTbl *) util_peek_list(priv->adapter->pmoal_handle,
                                               ...
                                               ra_list, MNULL, MNULL);
        while (ra_list &&
               (ra_list != (raListTbl *) & priv->wmm.tid_tbl_ptr[i].ra_list)) {
            if (memcmp(priv->adapter, ra_list->ra, mac, MLAN_MAC_ADDR_LENGTH)) {
                pkt_cnt += ra_list->total_pkts;
                ra_list->tx_pause = tx_pause;
            }
            ra_list = ra_list->pnext;
...
wlan_wmm_delete_tdls_ralist(pmlan_private priv, t_u8 * mac)
{
    ...
    for (i = 0; i < MAX_NUM_TID; ++i) {
        ...
        if (ra_list) {
            ...
            ra_list_ap = (raListTbl *) util_peek_list(pmadapter->pmoal_handle,
                                                      &priv->wmm.tid_tbl_ptr[i].
                                                      ra_list, MNULL, MNULL);
            while ((pmbuf =
                    (pmlan_buffer) util_peek_list(pmadapter->pmoal_handle,
            ...
            if (priv->wmm.tid_tbl_ptr[i].ra_list_curr == ra_list)
                priv->wmm.tid_tbl_ptr[i].ra_list_curr = ra_list_ap;
        }
    }
    ...
    for (i = 0; i < MAX_NUM_TID; ++i) {
        ...
        if (queuing_ra_based(priv)) {
            ...
            if (MTRUE == wlan_is_tdls_link_setup(status)) {
                ra_list->is_11n_enabled = is_station_11n_enabled(priv, ra);
                if (ra_list->is_11n_enabled)
                    ra_list->max_amsdu = get_station_max_amsdu_size(priv, ra);
...
wlan_hold_tdls_packets(pmlan_private priv, t_u8 * mac)
{
    ...
    for (i = 0; i < MAX_NUM_TID; ++i) {
        ra_list = (raListTbl *) util_peek_list(pmadapter->pmoal_handle,
                                               ...
                                               ra_list, MNULL, MNULL);
        if (ra_list) {
            while ((pmbuf = wlan_find_tdls_packets(priv, ra_list, mac))) {
                util_unlink_list(pmadapter->pmoal_handle,
                                 &ra_list->buf_head, (pmlan_linked_list) pmbuf,
                                 MNULL, MNULL);
                ra_list->total_pkts--;
...
wlan_restore_tdls_packets(pmlan_private priv, t_u8 * mac, tdlsStatus_e status)
{
    ...
    while ((pmbuf = wlan_find_packets_tdls_txq(priv, mac))) {
        util_unlink_list(pmadapter->pmoal_handle,
                         &priv->tdls_pending_txq, (pmlan_linked_list) pmbuf,
                         MNULL, MNULL);
        tid = pmbuf->priority;
        tid_down = wlan_wmm_downgrade_tid(priv, tid);
        ...
        PRINTM_NETINTF(MDATA, priv);
        PRINTM(MDATA, "ADD TDLS pkt %p (priority=%d) back to ra_list %p\n",
