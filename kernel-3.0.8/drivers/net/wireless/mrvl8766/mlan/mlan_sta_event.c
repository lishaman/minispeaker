...
wlan_parse_tdls_event(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    case TDLS_EVENT_TYPE_SETUP_REQ:
        if (sta_ptr == MNULL) {
            if ((sta_ptr =
                 wlan_add_station_entry(priv, tdls_event->peer_mac_addr))) {
    ...
    case TDLS_EVENT_TYPE_LINK_ESTABLISHED:
        if (sta_ptr) {
            ...
            ie_len = wlan_le16_to_cpu(tdls_event->u.ie_data.ie_length);
            if (ie_len) {
                pht_cap =
    ...
    case TDLS_EVENT_TYPE_SETUP_FAILURE:
        wlan_restore_tdls_packets(priv, tdls_event->peer_mac_addr,
                                  TDLS_SETUP_FAILURE);
        if (sta_ptr)
            wlan_delete_station_entry(priv, tdls_event->peer_mac_addr);
    ...
    case TDLS_EVENT_TYPE_CHAN_SWITCH_RESULT:
        ...
        if (tdls_event->u.switch_result.status == MLAN_STATUS_SUCCESS) {
            if (tdls_event->u.switch_result.current_channel ==
                ...
                if (pmadapter->tdls_status != TDLS_IN_BASE_CHANNEL) {
                    wlan_update_non_tdls_ralist(priv, tdls_event->peer_mac_addr,
                                                MFALSE);
                    pmadapter->tdls_status = TDLS_IN_BASE_CHANNEL;
                }
                       ...
                       TDLS_OFF_CHANNEL) {
                /* pause traffic to AP */
                if (pmadapter->tdls_status != TDLS_IN_OFF_CHANNEL) {
                    wlan_update_non_tdls_ralist(priv, tdls_event->peer_mac_addr,
                                                MTRUE);
...
wlan_process_sta_tx_pause_event(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_TX_PAUSE) {
            ...
            PRINTM(MCMND, "TDLS TxPause: " MACSTR " pause=%d, pkts=%d\n",
...
wlan_ops_sta_process_event(IN t_void * priv)
{
    ...
    if (eventcause != EVENT_PS_AWAKE && pmbuf->data_len > sizeof(eventcause))
    ...
    case EVENT_HEART_BEAT:
        PRINTM(MEVENT, "EVENT: Heart Beat\n");
        wlan_recv_event(pmpriv, MLAN_EVENT_ID_FW_HEART_BEAT, MNULL);
        break;
