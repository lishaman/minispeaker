...
wlan_11n_deaggregate_pkt(mlan_private * priv, pmlan_buffer pmbuf)
{
    ...
    /* Sanity test */
    if (pmadapter->usb_rx_deaggr.aggr_ctrl.enable) {
        max_rx_data_size = pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_max;
        if (pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_mode ==
            MLAN_USB_AGGR_MODE_NUM) {
            max_rx_data_size *=
                MAX(MLAN_USB_MAX_PKT_SIZE,
                    pmadapter->usb_rx_deaggr.aggr_ctrl.aggr_align);
            max_rx_data_size = MAX(max_rx_data_size, MLAN_USB_RX_DATA_BUF_SIZE);
        }
    }
                       ...
                       int headroom, int ptrindex)
{
    ...
    if ((pusb_tx_aggr = wlan_get_usb_tx_aggr_params(pmadapter, priv->port))) {
        ...
        ret =
            wlan_usb_host_to_card_aggr(pmadapter, pmbuf_aggr, &tx_param,
                                       pusb_tx_aggr);
    } else {
        pmadapter->data_sent = MTRUE;
        ret =
            pmadapter->callbacks.moal_write_data_async(pmadapter->pmoal_handle,
                                                       pmbuf_aggr, priv->port);
    }
    switch (ret) {
