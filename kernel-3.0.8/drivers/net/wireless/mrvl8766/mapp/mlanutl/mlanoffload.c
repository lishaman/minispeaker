...
profile_read_block(FILE * fp, profile_entry_t * p_head)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if ((ptr = strstr(line, "}")) != NULL) {
            ...
            if ((*ptr != '"') || ((eptr = strrchr(ptr + 1, '"')) == NULL)) {
            ...
            if (*ptr != '"') {
                ...
                if ((eptr = strrchr(ptr + 1, '"')) == NULL) {
                    fprintf(stderr, "passphrase not within quotes.\n");
                    break;
                }
            ...
            if (((eptr = strrchr(ptr + 1, '\r')) == NULL) &&
                ((eptr = strrchr(ptr + 1, '\n')) == NULL)) {
                fprintf(stderr, "missing EOL from the wep_key config\n");
                break;
            }
            ptr++;
            if (*ptr == '"') {
                ...
                *(eptr + 1) = '\0';
                ...
                for (i = 0; i < wep_len / 2; i++) {
                    ...
                    *(p_head->wep_key[key_cnt] + i) = (t_u8) byte;
                    ptr += 2;
                }
            }
            ...
            if ((eptr = strstr(ptr, "WPA-EAP-SHA256")) != NULL) {
                p_head->key_mgmt |= PROFILE_DB_KEY_MGMT_SHA256_IEEE8021X;
            }
            if ((eptr = strstr(ptr, "WPA-PSK-SHA256")) != NULL) {
                p_head->key_mgmt |= PROFILE_DB_KEY_MGMT_SHA256_PSK;
            }
...
profile_read_download(char *filename)
{
    ...
    while (fgets(line, sizeof(line), fp)) {
         ...
         */
        memset(pos, 0, (BUFFER_LENGTH - cmd_header_len - sizeof(t_u32)));
        ...
        if (p_head->passphrase_len) {
            ...
            memcpy((pos + cmd_len), &tempc, sizeof(t_u16));
            cmd_len += 2;
            memcpy((void *) (pos + cmd_len), p_head->passphrase, temp);
            cmd_len += temp;
        }
        ...
        for (i = 0; i < WEP_KEY_CNT; i++) {
            if (p_head->wep_key_len[i]) {
                ...
                tempc = cpu_to_le16(p_head->wep_key_len[i] + 1 + 1);
                memcpy((pos + cmd_len), &tempc, sizeof(t_u16));
                cmd_len += 2;
                *(pos + cmd_len) = i;
                cmd_len += 1;
                *(pos + cmd_len) = (i == p_head->wep_key_idx);
                ...
                memcpy((void *) (pos + cmd_len), p_head->wep_key[i], temp);
                cmd_len += temp;
            }
        }
...
chanrpt_graph(void)
{
    ...
    for (idx = 0; (unsigned int) idx < NELEMENTS(chanRpt); idx++) {
        ...
        chanrpt_getValues(idx + 1,
                          0, 100, &chanRpt[idx].anpi, &chanRpt[idx].chanLoad);
    }
...
chanrpt_graph_loop(boolean loopOnLoad, boolean loopOnAnpi, int loops)
{
    ...
    while (loopsLeft) {
        chanRpt[idx].chanNum = idx + 1;
        chanrpt_getValues(idx + 1,
                          0, 75, &chanRpt[idx].anpi, &chanRpt[idx].chanLoad);
                ...
                int argc, char *argv[])
{
    ...
    if (argv[3]) {
        for (idx = 0; idx < num_sub_cmds; idx++) {
            if (strncmp(argv[3], sub_cmd[idx].str, sub_cmd[idx].match_len) == 0) {
                invalid_cmd = FALSE;
...
process_measurement(int argc, char *argv[])
{
    ...
    if (argc == 7) {
        ...
        timing_tlv->header.type = cpu_to_le16(TLV_TYPE_MEASUREMENT_TIMING);
        timing_tlv->header.len = (sizeof(MrvlIETypes_MeasTiming_t)
    ...
    if (!strncmp(argv[3], "delete", sizeof("delete"))) {
        if (argc > 4) {
            if (strncmp(argv[4], "bssid=", strlen("bssid=")) == 0) {
                ...
                printf
                    ("Error: missing required option for command (ssid, bssid)\n");
                ret = -ENOMEM;
                goto done;
            }
...
process_chanrpt(int argc, char *argv[])
{
    ...
    if (argc == 6) {
        pChanRptReq->chanDesc.startFreq = atoi(argv[5]);
    }
...
process_assoc_timing(int argc, char *argv[])
{
    ...
    if (argc > 3) {
        ...
        switch (argc) {
        case 9:
            assoctiming->ReassocDiscMax = cpu_to_le16(atoi(argv[8]));
        ...
        case 7:
            ...
            /* No break, do everything below as well */
        case 6:
            assoctiming->HandShakeTimeout = cpu_to_le16(atoi(argv[5]));
            /* No break, do everything below as well */
        case 5:
            assoctiming->ReassocTimeout = cpu_to_le16(atoi(argv[4]));
            /* No break, do everything below as well */
        case 4:
...
process_iapp(int argc, char *argv[])
{
    ...
    pIappProxy->timeout_ms = a2hex_or_atoi(argv[3]);
...
display_channel(void)
{
    ...
    if (tmpfile == NULL) {
        ...
        while (fgets(result, sizeof(result), tmpfile)) {
            if ((sscanf(result, " Current Frequency=%d.%d ", &ghz, &mhz) == 2)
                || (sscanf(result, " Current Frequency:%d.%d ", &ghz, &mhz) ==
                    2)) {
                if (mhz < 10) {
                    mhz *= 100;
                } else if (mhz < 100) {
...
rateSort(RatePower_t rateList[], int numRates)
{
    ...
    while (inc > 0) {
        for (i = 0; i < numRates; i++) {
            ...
            while ((j >= inc) && (rateList[j - inc].rate > tmp.rate)) {
                memcpy(&rateList[j], &rateList[j - inc], sizeof(RatePower_t));
                j -= inc;
            }
...
process_chantrpcdisp(int startRate, int endRate)
{
    ...
    while (totalTlvBytes) {
        ...
        printf("%4d.%-3d ",
               pChanTrpcTlv->chanDesc.startFreq,
        ...
        for (idx = 0; idx < NELEMENTS(rateModPairs); idx++) {
            if ((rateModPairs[idx].rate >= startRate) &&
                ...
                for (modIdx = 0; modIdx < numModGroups; modIdx++) {
                    if (rateModPairs[idx].modGroup ==
                        pChanTrpcTlv->chanTrpcEntry[modIdx].modGroup) {
                        printf("%*d",
                               (numOut == 0) ? 3 : 6,
                               pChanTrpcTlv->chanTrpcEntry[modIdx].txPower);
                        numOut++;
                    }
                }
        ...
        pChanTrpcTlv = (MrvlIEtypes_ChanTrpcCfg_t *) ((t_u8 *) pChanTrpcTlv
                                                      + tlvSize);
...
process_txpowdisp(int argc, char *argv[])
{
    ...
    connected = pRfTxPower->current_level ? TRUE : FALSE;
    ...
    if (!connected) {
        ...
        process_chantrpcdisp(ratePower[12].rate, ratePower[20 - 1].rate);
    }
    ...
    if (!connected) {
        ...
        process_chantrpcdisp(ratePower[20].rate, ratePower[rates - 1].rate);
    }
