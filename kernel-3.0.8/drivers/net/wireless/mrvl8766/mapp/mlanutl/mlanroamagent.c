...
roamGetFailureCount(int argc, char *argv[])
{
    ...
    cmd_len = (S_DS_GEN + sizeof(HostCmd_DS_CMD_ROAMAGENT_STATISTICS_THRESHOLD)
    ...
    for (state = STATE_HARDROAM - 1; state < STATE_HARDROAM; state++) {
        ...
        while (i > 2) {
            ...
            case TLV_TYPE_FAILCOUNT:
                ...
                failTimeThresh =
                    le16_to_cpu(pFailureCount->fail_min_thresh_time_millisecs);
                break;
            }
    ...
    for (state = STATE_STABLE - 1; state < STATE_UNACCEPTABLE; state++) {
        ...
        while (i > 2) {
            ...
            case TLV_TYPE_RSSI_HIGH:
                high = *(t_s8 *) (tlvptr + sizeof(t_u16) + sizeof(t_u16));
                pHighRssiThreshold
                    = (MrvlIEtypes_BeaconHighRssiThreshold_t *) (tlvptr);
                tlvptr += (le16_to_cpu(pHighRssiThreshold->Header.len) +
                           sizeof(MrvlIEtypesHeader_t));
            ...
            case TLV_TYPE_RSSI_LOW:
                low = *(t_s8 *) (tlvptr + sizeof(t_u16) + sizeof(t_u16));
                pLowRssiThreshold
                    = (MrvlIEtypes_BeaconLowRssiThreshold_t *) (tlvptr);
                tlvptr += (le16_to_cpu(pLowRssiThreshold->Header.len) +
                           sizeof(MrvlIEtypesHeader_t));
            ...
            case TLV_TYPE_SNR_HIGH:
                ...
                tlvptr += (le16_to_cpu(pHighSnrThreshold->Header.len) +
                           sizeof(MrvlIEtypesHeader_t));
            ...
            case TLV_TYPE_SNR_LOW:
                ...
                tlvptr += (le16_to_cpu(pLowSnrThreshold->Header.len) +
                           sizeof(MrvlIEtypesHeader_t));
                  ...
                  pNborAssessmentParam)
{
    ...
    hostcmd->command = cpu_to_le16(HostCmd_CMD_ROAMAGENT_NEIGHBOR_ASSESSMENT);
    hostcmd->size = cpu_to_le16(cmd_len);
    hostcmd->seq_num = 0;
    hostcmd->result = 0;
...
roamGetMetrics(int argc, char *argv[])
{
    ...
    hostcmd->command = cpu_to_le16(HostCmd_CMD_ROAMAGENT_ADV_METRIC_THRESHOLD);
    hostcmd->size = cpu_to_le16(cmd_len);
    hostcmd->seq_num = 0;
    hostcmd->result = 0;
...
getControl(HostCmd_DS_CMD_ROAMAGENT_CONTROL * pcontrol)
{
    ...
    memcpy((void *) pcontrol,
           (void *) roamcontrolcmd, sizeof(HostCmd_DS_CMD_ROAMAGENT_CONTROL));
...
roamSetStatsThreshold(int argc, char *argv[], int type)
{
    ...
    if (type == RSSI_THRESHOLD) {
        ...
        if (low) {
            ...
            bcnlowsnr->Header.len = (sizeof(MrvlIEtypes_BeaconLowSnrThreshold_t)
                                     - sizeof(MrvlIEtypesHeader_t));
...
roamSetScanPeriod(int argc, char *argv[])
{
    ...
    cmd_len = S_DS_GEN + sizeof(HostCmd_DS_CMD_ROAMAGENT_NEIGHBOR_SCANPERIOD);
...
roamSetNborParams(int argc, char *argv[])
{
    ...
    pNborAssessment = (HostCmd_DS_CMD_ROAMAGENT_NEIGHBOR_ASSESSMENT *) pos;
    cmd_len = S_DS_GEN + sizeof(HostCmd_DS_CMD_ROAMAGENT_NEIGHBOR_ASSESSMENT);
    ...
    while (argc - j) {
        ...
        case 5:/** roamthresh */
            ...
            for (i = 0; (i < 12); i++) {
                if ((j >= argc) || (!isdigit(*argv[j]) && *argv[j] != '-')) {
                    temp = 0;
                } else {
                    temp = atoi(argv[j]);
                    j++;
                ...
                switch (i % 3) {
                case 0:
                    pNborAssessment->RoamThresh[i / 3].RssiHighLevel = temp;
                    break;

                case 1:
                    pNborAssessment->RoamThresh[i / 3].RssiLowLevel = temp;
                    break;

                case 2:
                    pNborAssessment->RoamThresh[i / 3].RssiNborDiff = temp;
                    break;
                }
            }
            break;
...
roamSetMetrics(int argc, char *argv[])
{
    ...
    cmd_len = S_DS_GEN + sizeof(HostCmd_DS_CMD_ROAMAGENT_ADV_METRIC_THRESHOLD);
