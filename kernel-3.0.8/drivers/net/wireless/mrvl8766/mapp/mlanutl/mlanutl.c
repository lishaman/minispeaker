        ...
        case HostCmd_CMD_LINK_STATS_SUMMARY:
            {
                HostCmd_DS_LINK_STATS_SUMMARY *linkstats =
                    (HostCmd_DS_LINK_STATS_SUMMARY *) (buf + S_DS_GEN);
                /* GET operation */
                ...
                printf("Tx AC VO:         %u [ %u ]\n",
                       ...
                       (int) linkstats->txQueueDelay[WMM_AC_VO] / 1000);
                printf("Tx AC VI:         %u [ %u ]\n",
                       le16_to_cpu(linkstats->txQueuePktCnt[WMM_AC_VI]),
                       (int) linkstats->txQueueDelay[WMM_AC_VI] / 1000);
                printf("Tx AC BE:         %u [ %u ]\n",
                       le16_to_cpu(linkstats->txQueuePktCnt[WMM_AC_BE]),
                       (int) linkstats->txQueueDelay[WMM_AC_BE] / 1000);
                printf("Tx AC BK:         %u [ %u ]\n",
                       le16_to_cpu(linkstats->txQueuePktCnt[WMM_AC_BK]),
                       (int) linkstats->txQueueDelay[WMM_AC_BK] / 1000);
                break;
            }
        default:
...
process_getlog(int argc, char *argv[])
{
    ...
    printf("mcasttxframe     %u\n" "failed           %u\n"
           ...
           stats->wep_icv_error[2], stats->wep_icv_error[3], stats->bcn_rcv_cnt,
           stats->bcn_miss_cnt);
...
process_getscantable(int argc, char *argv[])
{
    ...
    do {
        ...
        for (idx = 0; (unsigned int) idx < prsp_info->scan_number; idx++) {
            ...
            memset(scan_list_node, 0, sizeof(struct wlan_ioctl_get_scan_list));
            ...
            scan_list_node->fixed_buf.fixed_field_length = fixed_field_length;
            scan_list_node->fixed_buf.bss_info_length = bss_info_length;
            ...
            if (bss_info_length >=
                (sizeof(bss_info->tsf) + sizeof(bss_info->beacon_interval) +
                 sizeof(bss_info->cap_info))) {

                /* time stamp is 8 byte long */
                memcpy(bss_info->tsf, pcurrent, sizeof(bss_info->tsf));
                ...
                bss_info_length -= sizeof(bss_info->beacon_interval);
                /* capability information is 2 byte long */
            ...
            bss_info->wmm_cap = ' ';    /* M (WMM), C (WMM-Call Admission
                                           Control) */
            bss_info->wps_cap = ' ';    /* "S" */
            ...
            bss_info->priv_cap = bss_info->cap_info.privacy ? 'P' : ' ';
                ...
                case SSID:
                    if (*pelement_len &&
                        *pelement_len <= MRVDRV_MAX_SSID_LENGTH) {
                        memcpy(bss_info->ssid, pcurrent, *pelement_len);
                        bss_info->ssid_len = *pelement_len;
                    }
                    break;
            ...
            if ((argc > 3) && !strcmp(argv[3], "ch")) {
                ...
                /* Sort as per Signal Strength (descending order) (Default
                   case) */
            ...
            if (scan_list_head == NULL) {
                ...
                do {
                    if (sort_by_channel) {
                        curr_ch = curr->fixed_buf.fixed_fields.channel;
                        if (new_ch < curr_ch)
                            insert = 1;
                    } else {
                        curr_ss = 255 - curr->fixed_buf.fixed_fields.rssi;
                        if (new_ss > curr_ss) {
                            insert = 1;
                        }
                    }
                    if (insert) {
                        if (curr == scan_list_head) {
                            ...
                            scan_list_head->prev = scan_list_node;
                            scan_list_head = scan_list_node;
                        } else {
                            /* Insert the node to current position in list */
                            scan_list_node->prev = curr->prev;
                            scan_list_node->next = curr;
                            (curr->prev)->next = scan_list_node;
                            curr->prev = scan_list_node;
                        }
                        break;
                    }
                    if (curr->next == NULL) {
    ...
    for (curr = scan_list_head, idx = 0;
        ...
        /* "A" for Adhoc "I" for Infrastructure, "D" for DFS (Spectrum Mgmt) */
        ...
        for (ssid_idx = 0; ssid_idx < (int) bss_info->ssid_len; ssid_idx++) {
        ...
        if (argc > 3 && strcmp(argv[3], "tsf") == 0) {
            /* TSF is a u64, some formatted printing libs have trouble printing 
...
process_extcapcfg(int argc, char *argv[])
{
    ...
    if (argc == 4) {
        ...
        memcpy(buffer + strlen(CMD_MARVELL) + strlen(argv[2]), &ie, sizeof(ie));
        memcpy(buffer + strlen(CMD_MARVELL) + strlen(argv[2]) + sizeof(ie),
    ...
    if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
        perror("mlanutl");
        fprintf(stderr, "mlanutl: extended capabilities configure fail\n");
        if (cmd)
            free(cmd);
        if (buffer)
            free(buffer);
...
process_usbaggrctrl(int argc, char *argv[])
{
    ...
    if (argc == 3) {
        ...
        printf("  %s, Max = %d %s, Alignment = %d bytes, Timeout = %d us\n",
               ...
               (param->tx_aggr_ctrl.aggr_max < 512) ? "packets" : "bytes",
               param->tx_aggr_ctrl.aggr_align, param->tx_aggr_ctrl.aggr_tmo);
        printf("Rx aggregation:\n");
        printf("  %s, Max = %d %s, Alignment = %d bytes, Timeout = %d us\n",
               param->rx_deaggr_ctrl.enable ? "Enabled" : "Disabled",
               param->rx_deaggr_ctrl.aggr_max,
               (param->rx_deaggr_ctrl.aggr_max < 512) ? "packets" : "bytes",
...
process_tdls_setinfo(int argc, char *argv[])
{
    ...
    t_u16 cmd_len = 0, tlv_len = 0, tlv_len_rsn = 0, tlv_len_supp_chan =
        0, tlv_len_domain = 0;
    t_u16 no_of_sub_band = 0, no_of_supp_chan_sub_band = 0, pairwise_offset =
        0, akm_offset = 0, num_of_regulatory_class = 0, tlv_len_reg_class;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "CapInfo") == 0) {
            ...
            if ((tlv->u.qos_info_byte != 0) && (tlv->u.qos_info_byte != 0x0F)) {
            ...
            tlv->length = arg_num - 1;
            for (i = 0; i < tlv->length; i++) {
                tlv->ext_cap[i] = (t_u8) A2HEXDECIMAL(args[i + 1]);
            }
            endian_convert_tlv_header_out(tlv);
            ...
            tlv_ht_cap->ht_cap.ht_ext_cap = (t_u16) A2HEXDECIMAL(args[1]);
            tlv_ht_cap->ht_cap.ht_ext_cap =
                cpu_to_le16(tlv_ht_cap->ht_cap.ht_ext_cap);
        } else if (strcmp(args[0], "TxBfCapability") == 0) {
            tlv_ht_cap->ht_cap.tx_bf_cap = (t_u32) A2HEXDECIMAL(args[1]);
            ...
            tlv_ht_info->length = sizeof(tlvbuf_HTInfo_t) - TLVHEADER_LEN;
            cmd_len += tlv_len;
            ...
            tlv_ht_info->ht_info.field3 = (t_u16) A2HEXDECIMAL(args[1]);
            tlv_ht_info->ht_info.field3 =
                cpu_to_le16(tlv_ht_info->ht_info.field3);
        } else if (strcmp(args[0], "Field4") == 0) {
            tlv_ht_info->ht_info.field4 = (t_u16) A2HEXDECIMAL(args[1]);
            ...
            tlv->length = sizeof(tlvbuf_2040BSSCo_t) - TLVHEADER_LEN;
            tlv->bss_co_2040.bss_co_2040_value = (t_u8) A2HEXDECIMAL(args[1]);
            endian_convert_tlv_header_out(tlv);
            ...
            for (i = 0; i < PAIRWISE_CIPHER_SUITE_LEN; i++) {
                pairwise_cipher_suite[i] = (t_u8) A2HEXDECIMAL(args[i + 1]);
            }
            memcpy((t_u8 *) (rsn_ie->pairwise_cipher_suite +
                             (pairwise_index * PAIRWISE_CIPHER_SUITE_LEN)),
                   pairwise_cipher_suite, PAIRWISE_CIPHER_SUITE_LEN);
            pairwise_index++;
            pairwise_offset = pairwise_index * PAIRWISE_CIPHER_SUITE_LEN;
        } else if (strcmp(args[0], "AKMSuiteCount") == 0) {
            akm_count = (t_u16) atoi(args[1]);
            akm_count = cpu_to_le16(akm_count);
            memcpy((((t_u8 *) (&rsn_ie->akm_suite_count)) + pairwise_offset),
                   &akm_count, sizeof(t_u16));
            ...
            memcpy((t_u8 *) (rsn_ie->akm_suite + (akm_index * AKM_SUITE_LEN)
                             + pairwise_offset), akm_suite, AKM_SUITE_LEN);
            akm_index++;
            ...
            memcpy(((t_u8 *) (&(rsn_ie->rsn_capability))) + pairwise_offset +
                   akm_offset, &rsn_cap, sizeof(t_u16));
        } else if (strcmp(args[0], "PMKIDCount") == 0) {
            pmk_count = (t_u16) atoi(args[1]);
            pmk_count = cpu_to_le16(pmk_count);
            memcpy((((t_u8 *) (&rsn_ie->pmkid_count)) + pairwise_offset +
                    akm_offset), &pmk_count, sizeof(t_u16));
            ...
            memcpy((t_u8 *) (rsn_ie->pmkid_list + (pmkid_index * PMKID_LEN) +
                             pairwise_offset + akm_offset), pmkid, PMKID_LEN);
            pmkid_index++;
            ...
            /* Append a new TLV */
            supp_chan = (tlvbuf_SupportedChannels_t *) (buffer + cmd_len);
            ...
            supp_chan->subband[no_of_supp_chan_sub_band].start_chan =
                atoi(args[1]);
            ...
            tlv_len_supp_chan += sizeof(IEEEtypes_SupportChan_Subband_t);
            supp_chan->length += sizeof(IEEEtypes_SupportChan_Subband_t);
            ...
            /* Append a new TLV */
            reg_class = (tlvbuf_RegulatoryClass_t *) (buffer + cmd_len);
            ...
            reg_class->regulatory_class.cur_regulatory_class = atoi(args[1]);
            reg_class->length = 1;
            ...
            for (i = 0; i < num_of_regulatory_class; i++)
                reg_class->regulatory_class.regulatory_classes_list[i] =
                    (t_u8) A2HEXDECIMAL(args[i + 1]);
            ...
            memset(domain->country_code, ' ', sizeof(domain->country_code));
            memcpy(domain->country_code, country, strlen(country));
        } else if (strncmp(args[0], "FirstChannel", 12) == 0) {
            domain->sub_band[no_of_sub_band].first_chan = atoi(args[1]);
        } else if (strncmp(args[0], "NumberofChannels", 16) == 0) {
            domain->sub_band[no_of_sub_band].no_of_chan = atoi(args[1]);
        } else if (strncmp(args[0], "TxPower", 7) == 0) {
            domain->sub_band[no_of_sub_band].max_tx_pwr = atoi(args[1]);
            no_of_sub_band++;
    ...
    if (ret == MLAN_STATUS_SUCCESS) {
        ...
        if (resp_len > 0) {
            ...
            while (resp_len > IEEE_HEADER_LEN) {
                ...
                case TLV_TYPE_RATES:
                    printf("\tRates : ");
                    hexdump(NULL, ((t_u8 *) tlv) + IEEE_HEADER_LEN, tlv->len,
                            ' ');
                    break;
                ...
                tlv =
                    (IEEEtypes_Header_t *) ((t_u8 *) tlv + tlv->len +
                                            IEEE_HEADER_LEN);
            }
        }
...
process_tdls_setup(int argc, char *argv[])
{
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "PeerMAC") == 0) {
            ...
            param_buf->wait_time = cpu_to_le32(param_buf->wait_time);
        } else if (strcmp(args[0], "KeyLifetime") == 0) {
            param_buf->key_life_time = (t_u32) A2HEXDECIMAL(args[1]);
            param_buf->key_life_time = cpu_to_le32(param_buf->key_life_time);
        } else if (strcmp(args[0], "}") == 0 && cmd_found) {
            break;
        }
    }
...
process_tdls_link_status(int argc, char *argv[])
{
    ...
    if (ret == MLAN_STATUS_SUCCESS) {
        ...
        while (resp_len > 0 && no_of_links > 0) {
            ...
            printf("\t %s initiated link.\n", (link_ptr->link_flags & 0x01) ?
                   "Self" : "Peer");
            printf("\t Security %s.\n", (link_ptr->link_flags & 0x02) ?
                   "Enabled" : "Disabled");
            printf("\t Self PS status = %s.\n", (link_ptr->link_flags & 0x04) ?
                   "Sleep" : "Active");
            printf("\t Peer PS status = %s.\n", (link_ptr->link_flags & 0x08) ?
                   "Sleep" : "Active");
            ...
            if (link_ptr->traffic_status) {
                ...
                printf("%s",
                       (link_ptr->traffic_status & 0x01) ? "AC_BK, " : "");
                printf("%s",
                       (link_ptr->traffic_status & 0x02) ? "AC_BE, " : "");
                printf("%s",
                       (link_ptr->traffic_status & 0x04) ? "AC_VI, " : "");
                printf("%s", (link_ptr->traffic_status & 0x08) ? "AC_VO" : "");
                printf(".\n");
            }
            ...
            if (link_ptr->link_flags & 0x02) {
                ...
                printf("\t Security Method = %s\n",
                       (link_ptr->security_method == 1) ? "AES" : "None");
                printf("\t Key Lifetime = %d ms\n\t", link_ptr->key_lifetime);
                hexdump("Key", ((t_u8 *) link_ptr->key), link_ptr->key_length,
                        ' ');
                curr_link_len += sizeof(t_u32) + sizeof(t_u8) + sizeof(t_u8)
                    + link_ptr->key_length;
            }
            resp_len -= curr_link_len;
            link_ptr =
                (tdls_each_link_status *) (((t_u8 *) link_ptr) + curr_link_len);
            printf(".\n");
        }
...
process_tdls_channel_switch(int argc, char *argv[])
{
    ...
    if (argc != 4) {
        printf("ERR:Incorrect number of arguments.\n");
        printf
            ("Syntax: ./mlanutl mlanX tdls_channel_switch <config/tdls.conf>\n");
        exit(1);
    }
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "PeerMAC") == 0) {
            ...
            param_buf->regulatory_class = (t_u16) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "PrimaryChannel") == 0) {
            param_buf->primary_channel = (t_u16) A2HEXDECIMAL(args[1]);
            if (param_buf->band == BAND_BG_TDLS
            ...
            param_buf->secondary_channel_offset = (t_u16) A2HEXDECIMAL(args[1]);
            if (param_buf->secondary_channel_offset != 0
                && param_buf->secondary_channel_offset != SECOND_CHANNEL_ABOVE
            ...
            param_buf->switch_timeout = (t_u16) A2HEXDECIMAL(args[1]);
...
process_tdls_stop_channel_switch(int argc, char *argv[])
{
    ...
    if (argc != 4) {
        printf("ERR:Incorrect number of arguments.\n");
        printf
            ("Syntax: ./mlanutl mlanX tdls_stop_channel_switch <config/tdls.conf>\n");
        exit(1);
    }
...
process_tdls_cs_params(int argc, char *argv[])
{
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "UnitTime") == 0) {
            ...
            param_buf->threshold_otherlink = (t_u8) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "ThresholdDirectLink") == 0) {
            param_buf->threshold_directlink = (t_u8) A2HEXDECIMAL(args[1]);
        } else if (strcmp(args[0], "}") == 0 && cmd_found) {
            break;
        }
    }
...
process_tdls_debug(int argc, char *argv[])
{
    ...
    /* same link */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug setup_existing_link <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
    /* fail_setup_confirm */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug fail_setup_confirm <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
    /* setup prohibited */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug setup_with_prohibited <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
    /* setup higher/lower mac */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug higher_lower_mac <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
    /* ignore key lifetime expiry */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug ignore_key_expiry <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
    /* allow weak security */
        ...
        if (argc < 5) {
            printf("ERR:Incorrect number of arguments.\n");
            printf
                ("Syntax: ./mlanutl mlanX tdls_debug allow_weak_security <0/1>\n");
            exit(1);
        }
        value = (t_u16) A2HEXDECIMAL(argv[4]);
    }
...
process_delts(int argc, char *argv[])
{
    ...
    copy_len = sizeof(deltsReq) - sizeof(deltsReq.ieData) + deltsReq.ieDataLen;
...
process_linkstats(int argc, char *argv[])
{
    ...
    cmd_len = sizeof(HostCmd_DS_GEN) + sizeof(HostCmd_DS_LINK_STATS_SUMMARY);
