...
print_event_wifidirect_generic(t_u8 * buffer, t_u16 size)
{
    ...
    if (type == 2) {
        ...
        case 2:
            printf
                ("Packet processing state = Insufficient information, Dropped\n");
            break;
        case 3:
    ...
    if (action == WIFIDIRECT_DISCOVERY_REQUEST_ACTION) {
        ...
        if (wifidirect_disc_resp->service_protocol == 1) {
            ...
            /* 
               printf("\t\t DNS = "); dns_len =
               uap_le16_to_cpu(wifidirect_disc_resp->vendor_len) -
               (WIFIDIRECT_DISCOVERY_BONJOUR_FIXED_LEN + 1); for( i=0; i <
               dns_len; i++)
               printf("%c",*(wifidirect_disc_resp->disc_resp.u.bonjour.dns +
               i)); memcpy(&dns_type,
               (&wifidirect_disc_req->disc_query.u.bonjour.dns_type + dns_len), 
               sizeof(dns_type)); dns_type = uap_le16_to_cpu(dns_type);
               printf("\n\t\t DNS Type = %d\n", dns_type); printf("\t\t Version 
...
print_event_tdls_generic(t_u8 * buffer, t_u16 size)
{
    ...
    case TDLS_EVENT_TYPE_CHAN_SWITCH_RESULT:
        if (tdls_event->u.switch_result.status) {
            printf
                ("TDLS Channel Switch Failed! current channel=%d Reasoncode %d\n",
    ...
    if (uap_le16_to_cpu(tdls_event->event_type) ==
        ...
        case 25:
            printf
                ("Reason Code = Peer station is unreachable on direct link\n");
            break;
        case 26:
        ...
        case 212:
            printf
                ("Reason Code = Clearing TDLS connections due to deauth/disassoc\n");
            break;
        default:
            printf("Reason Code = %d\n",
                   uap_le16_to_cpu(tdls_event->u.reason_code));
...
print_event(event_header * event, t_u16 size)
{
    ...
    case EVENT_TDLS_GENERIC:
        print_event_tdls_generic(event->event_data, size - EVENT_ID_LEN);
        break;
    ...
    case EVENT_WEP_ICV_ERROR:
        print_event_wep_icv_error(event->event_data, size - EVENT_ID_LEN);
        break;
...
#define CUS_EVT_STA_CONNECTED           "EVENT=STA_CONNECTED"
        if (!strncmp
            ...
            print_mac((t_u8 *) event + strlen(CUS_EVT_STA_CONNECTED) + 1);
            printf("\n");
            break;
        }
    ...
    for (i = 0; i < no_of_sk; i++) {
        if (sk_fd[i] > 0) {
            if (FD_ISSET(sk_fd[i], &rfds)) {
                ...
                recv_buf[i].flag = 1;
                recv_buf[i].length =
                    read_event_netlink_socket(sk_fd[i], recv_buf[i].buffer, nlh,
                                              msg);
...
main(int argc, char *argv[])
{
    ...
    for (i = 0; i < no_of_sk; i++) {
        ...
        if (no_of_sk == 1) {
            ...
            if (netlink_num[i] < 0) {
                printf
                    ("ERR:Could not get netlink socket. Invalid device number.\n");
        ...
        if (netlink_num[i] >= 0) {
            ...
            if (bind(nl_sk[i], (struct sockaddr *) &src_addr, sizeof(src_addr))
                < 0) {
    ...
    while (1) {
        ...
        for (i = 0; i < no_of_sk; i++) {
            if (evt_recv_buf[i].flag == 1) {
                ...
                memcpy(&event_id, evt_recv_buf[i].buffer, sizeof(event_id));
                if (((event_id & 0xFF000000) == 0x80000000) ||
                    ((event_id & 0xFF000000) == 0)) {
                    event = (event_header *) (evt_recv_buf[i].buffer);
                } else {
                    memset(if_name, 0, IFNAMSIZ + 1);
                    memcpy(if_name, evt_recv_buf[i].buffer, IFNAMSIZ);
                    printf("EVENT for interface %s\n", if_name);
                    event =
                        (event_header *) ((t_u8 *) (evt_recv_buf[i].buffer) +
                                          IFNAMSIZ);
                    ret -= IFNAMSIZ;
