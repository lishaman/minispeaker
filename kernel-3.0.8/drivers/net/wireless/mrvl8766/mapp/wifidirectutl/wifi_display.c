...
wifidisplaycmd_config(int argc, char *argv[])
{
    ...
    if (argc >= 3) {
        ...
        wifidisplay_file_params_config(argv[2], argv[1], buf
                                       ...
                                       sizeof(custom_ie), &ie_len_wifidisplay);
        if (argc == 4) {
        ...
        ie_len =
            ie_len_wifidisplay + sizeof(tlvbuf_custom_ie) + sizeof(custom_ie);
                               ...
                               t_u8 * pbuf, t_u16 * ie_len_wifidisplay)
{
    ...
    t_u8 total_num_device_info __attribute__ ((__unused__));    /* For future
                                                                   use */
...
wifidisplay_cmd_status(int argc, char *argv[])
{
    ...
    if (argc == 3) {
        if ((ISDIGIT(argv[2]) == 0) || (atoi(argv[2]) < 0) ||
            (atoi(argv[2]) > 3)) {
            printf
                ("ERR:Illegal wfd mode %s. Must be in range from '0' to '3'.\n",
                 argv[2]);
            print_wifi_display_status_usage();
            return;
        }
...
is_wifidisplay_input_valid(display_valid_inputs cmd, int argc, char *argv[])
{
    ...
    case WFD_DEVICE_INFO:
        ...
        if ((ISDIGIT(argv[0]) == 0) || (atoi(argv[0]) < 0)
            || (atoi(argv[0]) > ((1 << 10) - 1))) {
            printf("ERR:Coupled sink paramater must be > 0 and < %d",
                   ((1 << 10) - 1));
            return FAILURE;
        }
        ...
        if ((atoi(argv[0]) & 48) > 16) {
            printf
                ("ERR:Coupled sink paramater must not have bit 4 and 5 equal to 10 or 11 ");
            return FAILURE;
        }
        break;
    ...
    case WFD_DEVICE_THROUGHPUT:
        if ((ISDIGIT(argv[0]) == 0) || (atoi(argv[0]) < 0)
            || (atoi(argv[0]) > ((1 << 16) - 1))) {
            printf("ERR:Paramater must be > 0 and < %d", ((1 << 16) - 1));
            return FAILURE;
        }
        break;
    ...
    if (display_ie_index < (MAX_MGMT_IE_INDEX - 1)) {
        ...
        if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
            ...
            while (ie_len_wfd > WFD_IE_HEADER_LEN) {
                ...
                if (*wfd_ptr == TLV_TYPE_WIFIDISPLAY_DEVICE_INFO) {
                    tlvbuf_wfdisplay_device_info *wfd_tlv =
                        (tlvbuf_wfdisplay_device_info *) wfd_ptr;
                    new_wfd_dev_info = ntohs(new_wfd_dev_info);
                    memcpy((t_u8 *) & wfd_tlv->display_device_info,
                           (t_u8 *) & new_wfd_dev_info, sizeof(t_u16));
                    break;
                }
            ...
            wifiDisplay_ie_config((t_s16 *) & display_ie_index,
                                  ie_len_wfd_org,
                                  (t_u8 *) ie_ptr - sizeof(tlvbuf_custom_ie));
        }
    }
    return;
}
    ...
    if (display_ie_index < (MAX_MGMT_IE_INDEX - 1)) {
        ...
        if (tlv->tag == MRVL_MGMT_IE_LIST_TLV_ID) {
            ...
            while (ie_len_wfd > WFD_IE_HEADER_LEN) {
                ...
                len = ntohs(len);
                /* check capability type */
                if (*wfd_ptr == TLV_TYPE_WIFIDISPLAY_COUPLED_SINK) {
                    tlvbuf_wfdisplay_coupled_sink *wfd_tlv =
                        (tlvbuf_wfdisplay_coupled_sink *) wfd_ptr;
                    wfd_tlv->coupled_sink = new_wfd_coupled_sink_bitmap;
                    break;
                }
                wfd_ptr += len + WFD_IE_HEADER_LEN;
                ie_len_wfd -= len + WFD_IE_HEADER_LEN;
