    ...
    do {
        ...
        if (ioctl(sockfd, MLAN_ETH_PRIV, &ifr)) {
            if (errno == EAGAIN) {
                ...
                perror("mlan2040coex");
                fprintf(stderr, "mlan2040coex: getscantable fail\n");
                ret = MLAN_STATUS_FAILURE;
            }
            goto done;
        }
        ...
        for (idx = 0; (unsigned int) idx < prsp_info->scan_number; idx++) {
            ...
            if (!ht_cap_present || intol_bit_is_set) {
                ...
                for (j = 0; j < i; j++) {
                    if (leg_ap_chan_list[j].chan_num == fixed_fields.channel) {
                        already_listed = TRUE;
                        if (intol_bit_is_set)
                            leg_ap_chan_list[j].is_intol_set = intol_bit_is_set;
                        break;
                    }
                }
                if (!already_listed) {
                    /* add the channel in list */
                    leg_ap_chan_list[i].chan_num = fixed_fields.channel;
...
process_setuserscan(void)
{
    ...
    if (scan_cfg[0] == SCAN_TYPE_ACTIVE)
        ...
        scan_req.chan_list[0].scan_time =
            (t_u32) le16_to_cpu(cur_obss_scan_param.obss_scan_passive_total);
...
{
    if (!strncmp
        (CUS_EVT_AP_CONNECTED, (char *) event, strlen(CUS_EVT_AP_CONNECTED))) {
        if (strlen(if_name))
...
read_event(int nl_sk, struct msghdr *msg, struct timeval *ptv)
{
    ...
    if (ret == MLAN_STATUS_FAILURE) {
        ...
        if (assoc_flag && is_ht_ap) {
            /* Timeout. Try again after BSS channel width triger scan interval
               when the STA is connected with a HT AP */
            ptv->tv_sec =
                (t_u32) le16_to_cpu(cur_obss_scan_param.
                                    bss_chan_width_trigger_scan_int);
        } else {
            /* Timeout. Try again after default duration when the STA is not
               connected with a HT AP */
            ptv->tv_sec = DEFAULT_SCAN_INTERVAL;
        }
        ptv->tv_usec = 0;
        goto done;
                 ...
                 t_u8 reg_class, t_u8 is_intol_ap_present)
{
    ...
    switch (hostcmd_idx) {
    case CMD_2040COEX:
        prepare_coex_cmd_buff(buffer + strlen(CMD_MARVELL) + strlen("hostcmd"),
                              chan_list, chan_num, reg_class,
                              is_intol_ap_present);
        break;
    default:
        break;
