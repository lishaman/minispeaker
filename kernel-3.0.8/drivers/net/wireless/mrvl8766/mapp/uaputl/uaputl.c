...
apcmd_deepsleep(int argc, char *argv[])
{
    ...
    if (ioctl(sockfd, UAP_IOCTL_CMD, &ifr)) {
        if (argc)
            printf
                ("ERR: deep sleep must be disabled before changing idle time\n");
        else {
            perror("");
...
apcmd_tx_rate_cfg(int argc, char *argv[])
{
    ...
    if (argc) {
        ...
        tx_rate_config.rate = A2HEXDECIMAL(argv[0]);
        ...
        if (UAP_SUCCESS == get_sys_cfg_11n(&htcap)) {
            if (htcap.supported_mcs_set[0] && (status == UAP_BSS_START)) {
                ...
                if ((tx_rate_config.rate == UAP_RATE_INDEX_MCS32) &&
                    (!(IS_11N_40MHZ_ENABLED(htcap.ht_cap_info)))) {
                    printf
...
apcmd_sys_config_profile(int argc, char *argv[])
{
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "country") == 0) {
            ...
            strncpy(country_80211d, args[1], sizeof(country_80211d) - 1);
        ...
        if (strcmp(args[0], "HT_MCS_MAP") == 0) {
            if (enable_11n <= 0) {
                printf
                    ("ERR: Enable11n parameter should be set before HT_MCS_MAP.\n");
                ret = UAP_FAILURE;
                goto done;
            }
            if (0 == get_fw_info(&fw)) {
                /* Check upper nibble of MCS support value and block MCS_SET_1 
                   when 2X2 is not supported by the underlying hardware */
                if (((fw.hw_dev_mcs_support & 0xf0) < STREAM_2X2_MASK) &&
            ...
            memcpy(tlv->ht_cap.supported_mcs_set, &supported_mcs_set,
                   sizeof(t_u32));
        }
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            while ((unsigned int) size >= sizeof(tlvbuf_header)) {
                ...
                case 4:
                    ...
                    printf("%d", (unsigned int) uap_le32_to_cpu(byte4));
                    break;
                default:
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            if (!custom_ie_present) {
                printf("\nCustom IE settings:\n");
                if (apcmd_sys_cfg_custom_ie(1, argv_dummy) == UAP_FAILURE) {
                    return UAP_FAILURE;
                }
            }
        } else {
...
update_domain_info(char *country_80211d, int band)
{
    ...
    if ((ret == UAP_SUCCESS) && (cmd_buf->result == CMD_SUCCESS) &&
        (cmd_buf->domain.country_code[0] ||
         cmd_buf->domain.country_code[1] || cmd_buf->domain.country_code[2])) {
    ...
    if (band == BAND_A)
        printf
            ("Enabling 11d for 11h operation and setting country code to %s\n",
             country);
    cmd_len = sizeof(apcmdbuf_cfg_80211d);
    /** Get domain information from the file */
    no_of_sub_band = parse_domain_file(country, band, sub_bands);
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header) && (ret != UAP_FAILURE)) {
        ...
        case MRVL_CHANNELCONFIG_TLV_ID:
            ...
            if ((!state_80211d) && (channel_tlv->chan_number == MAX_CHANNELS_BG)
                ...
                printf
                    ("ERR: Invalid channel 14, 802.11d disabled, country code JP!\n");
                return UAP_FAILURE;
            }
...
#define JP_REGION_0xFE 0xFE
            if (fw.region_code == JP_REGION_0xFE &&
                ...
                printf
                    ("ERR:Channel 12 or 13 is not allowed for region code 0xFE\n");
                return UAP_FAILURE;
            }
        ...
        case MRVL_CHANNELLIST_TLV_ID:
            ...
            for (i = 0; i < chan_list_len; i++) {
                ...
                if ((!state_80211d) &&
                    ...
                    printf
                        ("ERR: Invalid scan channel 14, 802.11d disabled, country code JP!\n");
                    return UAP_FAILURE;
                }
                ...
                if (fw.region_code == JP_REGION_0xFE &&
                    ...
                    printf
                        ("ERR:Scan Channel 12 or 13 is not allowed for region code 0xFE\n");
                    return UAP_FAILURE;
                }
    ...
    if (state_80211d && acs_mode_enabled) {
        ...
        for (i = 0; i < chan_list_len; i++) {
            ...
            if ((pchan_list->chan_number < MAX_CHANNELS_BG) &&
                (scan_channels_band == BAND_A)) {
                strncpy(country_80211d, "JP ", sizeof(country_80211d) - 1);
            }
            if (check_channel_validity_11d
    ...
    if ((!acs_mode_enabled && (channel_tlv_band == BAND_A))
        ...
        state_80211h = 1;
        if (sg_snmp_mib
            (ACTION_SET, OID_80211H_ENABLE, sizeof(state_80211h), &state_80211h)
            == UAP_FAILURE) {
            return UAP_FAILURE;
        }
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if ((strcmp(args[0], "Aifsn") == 0) && is_wmm_parameters) {
            ...
            printf("wmm_para.ac_params[%d].aci_aifsn.aci = %x\n", ac,
                   bss->wmm_para.ac_params[ac].aci_aifsn.aci);
        }
        ...
        if (strcmp(args[0], "HT_MCS_MAP") == 0) {
            ...
            supported_mcs_set = (t_u32) A2HEXDECIMAL(args[1]);
            supported_mcs_set = uap_cpu_to_le32(supported_mcs_set);
            memcpy(bss->supported_mcs_set, &supported_mcs_set, sizeof(t_u32));
        }
        if (strcmp(args[0], "Enable2040Coex") == 0) {
            if ((ISDIGIT(args[1]) == 0) || (atoi(args[1]) < 0) ||
                (atoi(args[1]) > 1)) {
...
apcmd_coex_config_profile(int argc, char *argv[])
{
    ...
    t_u16 sco_prot_qtime[4] = { 0, 0, 0, 0 }, sco_prot_rate = 0, sco_acl_freq =
        0;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (is_coex_acl_config == 1) {
            ...
            if (strcmp(args[0], "}") == 0) {
                ...
                coex_common_tlv->ap_bt_coex = uap_cpu_to_le32(ap_coex_enable);
        ...
        } else if ((strcmp(args[0], "APBTCoex") == 0) && is_coex_common_config) {
            if (is_input_valid(COEX_COMM_AP_COEX, arg_num - 1, args + 1) !=
                UAP_SUCCESS) {
                ret = UAP_FAILURE;
                goto done;
            }
    ...
    if (ret == UAP_SUCCESS) {
        /* Verify response */
        if (cmd_buf->cmd_code != (HostCmd_ROBUST_COEX | APCMD_RESP_CHECK)) {
            printf("ERR:Corrupted response!\n");
            ret = UAP_FAILURE;
            goto done;
        }
...
apcmd_sys_cfg_custom_ie(int argc, char *argv[])
{
    ...
    if (argc == 1 && max_mgmt_ie_print == 0) {
        /* Print buffer sizes only if (argc == 1) i.e. get all indices one by
           one && (max_mgmt_ie_print == 0) i.e. max mgmt IE is not printed via 
           sys_config cmd */
        print_flag = 1;
    }
    ...
    case 2:
        if ((atoi(argv[1]) < 0) || (atoi(argv[1]) >= max_mgmt_ie)) {
            printf
                ("ERR:Illegal index %s. Must be either greater than or equal to 0 and less than %d for Get Operation \n",
                 argv[1], max_mgmt_ie);
            print_sys_cfg_custom_ie_usage();
            return UAP_FAILURE;
        }
...
get_tdls_ext_cap(tlvbuf_tdls_ext_cap * ext_cap_tlv)
{
    ...
    if (ret == UAP_SUCCESS) {
        ...
        /* Copy response */
        if (cmd_buf->result == CMD_SUCCESS) {
            memcpy(ext_cap_tlv, tlv, sizeof(tlvbuf_tdls_ext_cap) + tlv->length);
        } else {
            ret = UAP_FAILURE;
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (is_tdls_config) {
            if (strcmp(args[0], "tdls_prohibit") == 0) {
                ...
                ext_cap_tlv->ext_cap[4] &= (~TDLS_PROHIBIT_MASK);
                ext_cap_tlv->ext_cap[4] |= (atoi(args[1]) << TDLS_PROHIBIT);
            }
            if (strcmp(args[0], "tdls_channel_switch_prohibit") == 0) {
                if ((ISDIGIT(args[1]) != UAP_SUCCESS) || (atoi(args[1]) < 0) ||
                    (atoi(args[1]) > 1)) {
                    printf("ERR: Invalid %s value!\n", args[0]);
                    ret = UAP_FAILURE;
                ...
                ext_cap_tlv->ext_cap[4] &= (~TDLS_CHANNEL_SWITCH_PROHIBIT_MASK);
                ext_cap_tlv->ext_cap[4] |=
                    (atoi(args[1]) << TDLS_CHANNEL_SWITCH_PROHIBIT);
            }
        }
    }
  done:
...
apcmd_sys_cfg_tdls_ext_cap(int argc, char *argv[])
{
    ...
    tlv = (tlvbuf_tdls_ext_cap *) (buffer + sizeof(apcmdbuf_sys_configure));
    if (argc == 1) {
        /* Get current TDLS extended capability setting from FW and parse new
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            if (argc == 0) {
                printf("TDLS prohibited = %s\n",
                       (tlv->ext_cap[4] & TDLS_PROHIBIT_MASK) ? "YES" : "NO");
                printf("TDLS Channel switch prohibited = %s\n",
                       (tlv->
                        ext_cap[4] & TDLS_CHANNEL_SWITCH_PROHIBIT_MASK) ? "YES"
                       : "NO");
            } else {
...
static command_table ap_command[] = {
    ...
    {"hssetpara", apcmd_hssetpara, "\t\tSet/get uAP host sleep parameters."},
...
is_input_valid(valid_inputs cmd, int argc, char *argv[])
{
    ...
    case PKTFWD:
        if (argc != 1) {
            ...
            printf
                ("ERR:Illegal PKTFWD parameter %s. Must be within '0' and '15'.\n",
                 argv[0]);
            ret = UAP_FAILURE;
        }
        break;
...
print_tlv(t_u8 * buf, t_u16 len)
{
    ...
    while (tlv_buf_left >= (int) sizeof(tlvbuf_header)) {
        ...
        case MRVL_PKT_FWD_CTL_TLV_ID:
            ...
            printf("%s handles packet forwarding -\n",
                   ((pkt_fwd_tlv->pkt_fwd_ctl & PKT_FWD_FW_BIT) ==
                    0) ? "Host" : "Firmware");
            printf("\tIntra-BSS broadcast packets are %s\n",
                   ((pkt_fwd_tlv->pkt_fwd_ctl & PKT_FWD_INTRA_BCAST) ==
                    0) ? "allowed" : "denied");
            printf("\tIntra-BSS unicast packets are %s\n",
                   ((pkt_fwd_tlv->pkt_fwd_ctl & PKT_FWD_INTRA_UCAST) ==
                    0) ? "allowed" : "denied");
            printf("\tInter-BSS unicast packets are %s\n",
                   ((pkt_fwd_tlv->pkt_fwd_ctl & PKT_FWD_INTER_UCAST) ==
                    0) ? "allowed" : "denied");
            break;
        ...
        case MRVL_MAX_MGMT_IE_TLV_ID:
            ...
            max_mgmt_ie_tlv->count = uap_le16_to_cpu(max_mgmt_ie_tlv->count);
            for (i = 0; i < max_mgmt_ie_tlv->count; i++) {
                max_mgmt_ie_tlv->info[i].buf_size =
                    uap_le16_to_cpu(max_mgmt_ie_tlv->info[i].buf_size);
                max_mgmt_ie_tlv->info[i].buf_count =
                    uap_le16_to_cpu(max_mgmt_ie_tlv->info[i].buf_count);
                printf("buf%d_size = %d\n", i,
                       max_mgmt_ie_tlv->info[i].buf_size);
                printf("number of buffers = %d\n",
                       max_mgmt_ie_tlv->info[i].buf_count);
        ...
        case MRVL_BT_COEX_COMMON_CFG_TLV_ID:
            ...
            coex_common_tlv = (tlvbuf_coex_common_cfg *) pcurrent_tlv;
        ...
        case HT_CAPABILITY_TLV_ID:
            ...
            if (!ht_cap_tlv->ht_cap.supported_mcs_set[0]) {
                ...
                for (i = 0; i < MCS_SET_LEN; i++) {
                    printf("0x%x ", ht_cap_tlv->ht_cap.supported_mcs_set[i]);
                }
                printf("\n");
            }
            break;
...
uap_ioctl_reroute(t_u8 * cmd, t_u16 * size, t_u16 buf_size, int *reroute_ret)
{
    ...
    case HostCmd_SNMP_MIB:
        ...
        cmd_action = hcmd_snmp->action;
        /* reroute CMD_SNMP_MIB: SET */
        if (cmd_action == ACTION_SET) {
            reroute = 1;
            buf_len = sizeof(snmp_mib_param) + uap_le16_to_cpu(tlv->len);
        }
        break;
    case HostCmd_CMD_802_11D_DOMAIN_INFO:
        hcmd_domain = (apcmdbuf_cfg_80211d *) cmd;
        cmd_action = hcmd_domain->action;
        /* reroute CMD_DOMAIN_INFO: SET */
        if (cmd_action == ACTION_SET) {
            reroute = 1;
            buf_len = sizeof(domain_info_param) + (buf_size
                                                   -
                                                   sizeof(apcmdbuf_cfg_80211d) +
                                                   sizeof(domain_param_t));
        }
        break;
    }
...
uap_ioctl(t_u8 * cmd, t_u16 * size, t_u16 buf_size)
{
    ...
    if (strncmp(dev_name, "wfd", 3))
#endif
        if (uap_ioctl_reroute(cmd, size, buf_size, &reroute_ret) == UAP_SUCCESS) {
            return reroute_ret;
        }
