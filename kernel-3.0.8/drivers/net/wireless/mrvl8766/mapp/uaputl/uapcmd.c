...
check_channel_validity_11d(int channel, int band, int set_domain)
{
    ...
    if (ret == UAP_SUCCESS) {
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            /* copy third character of country code here as domain file cannot
               handle it */
...
apcmd_sys_cfg_channel_ext(int argc, char *argv[])
{
    ...
    if (argc == 0) {
        ...
        if (argc == 1) {
            ...
            if (atoi(argv[1]) == 0) {
                tlv->band_config_type |= BAND_CONFIG_2_4GHZ;
            } else {
                tlv->band_config_type |= BAND_CONFIG_5GHZ;
            }
            if (argc == 3) {
    ...
    if (argc) {
        ...
        if ((output[0][0] != -1) && (output[0][1] > output[0][0])) {
            ...
            for (j = (output[0][0] + 1); j < output[0][1]; j++) {
                ...
                memset(argv_rate[i], 0, sizeof(char) * (strlen(argv[j]) + 1));
        ...
        if ((output[1][0] != -1) && (output[1][1] > output[1][0])) {
            ...
            argv_mrate[0] =
                (char *) malloc(sizeof(char) * (strlen(argv[j]) + 1));
            memset(argv_mrate[0], 0, sizeof(char) * (strlen(argv[j]) + 1));
            strncpy(argv_mrate[0], argv[output[1][0] + 1], strlen(argv[j]));
        }
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            if (argc == 0) {
                ...
                printf("\tIntra-BSS broadcast packets are %s\n",
                       ((tlv->pkt_fwd_ctl & PKT_FWD_INTRA_BCAST) ==
                        0) ? "allowed" : "denied");
                printf("\tIntra-BSS unicast packets are %s\n",
                       ((tlv->pkt_fwd_ctl & PKT_FWD_INTRA_UCAST) ==
                        0) ? "allowed" : "denied");
                printf("\tInter-BSS unicast packets are %s\n",
                       ((tlv->pkt_fwd_ctl & PKT_FWD_INTER_UCAST) ==
                        0) ? "allowed" : "denied");
            } else {
...
apcmd_sys_cfg_wep_key(int argc, char *argv[])
{
    ...
    for (i = 0; i < number_of_keys; i++) {
        ...
        tmp_buffer =
            realloc(buffer, (cmd_len + sizeof(tlvbuf_wep_key) + key_len));
        if (!tmp_buffer) {
...
apcmd_sys_cfg_max_sta_num(int argc, char *argv[])
{
    ...
    if (argc) {
        if (get_max_sta_num_supported(&max_sta_num_supported) == UAP_FAILURE) {
            return UAP_FAILURE;
        }
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            if (argc == 0) {
                printf
                    ("ERR:Could not get configured max station count and supported max station count!\n");
            } else {
...
apcmd_sys_cfg_sticky_tim_config(int argc, char *argv[])
{
    ...
    /* Check arguments */
    if (argc && (is_input_valid(STICKYTIMCONFIG, argc, argv) != UAP_SUCCESS)) {
        print_sys_cfg_sticky_tim_config_usage();
        return UAP_FAILURE;
    }
    /* Initialize the command length */
    cmd_len = sizeof(apcmdbuf_sys_configure) + sizeof(tlvbuf_sticky_tim_config);
    /* Initialize the command buffer */
    buffer = (t_u8 *) malloc(buf_len);
    ...
    cmd_buf = (apcmdbuf_sys_configure *) buffer;
    tlv =
        (tlvbuf_sticky_tim_config *) (buffer + sizeof(apcmdbuf_sys_configure));
...
apcmd_sys_cfg_sticky_tim_sta_mac_addr(int argc, char *argv[])
{
    ...
    /* Initialize the command length */
    cmd_len =
        sizeof(apcmdbuf_sys_configure) + sizeof(tlvbuf_sticky_tim_sta_mac_addr);
    ...
    cmd_buf = (apcmdbuf_sys_configure *) buffer;
    tlv =
        (tlvbuf_sticky_tim_sta_mac_addr *) (buffer +
                                            sizeof(apcmdbuf_sys_configure));
    ...
    if (argc < 2) {
        ...
        if (argc == 0) {
            tlv->length = 0;
            cmd_len -= sizeof(tlvbuf_sticky_tim_sta_mac_addr) - TLVHEADER_LEN;
        }
        if ((argc == 1) &&
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            if (argc < 2) {
                ...
                while (tlv_get_len > 0) {
                    ...
                    printf("\ncontrol = %x\n", tlv->control);
                    tlv++;
                    tlv_get_len -= sizeof(tlvbuf_sticky_tim_sta_mac_addr);
                }
            } else {
                printf
                    ("sticky TIM configuration for given station is successful\n");
            }
        } else {
            if (argc < 2) {
                printf
                    ("ERR:Could not get sticky TIM configuration for station(s)!\n");
            } else {
                printf
                    ("ERR:Could not set sticky TIM configuration for given station!\n");
            }
            ret = UAP_FAILURE;
        }
    } else {
...
apcmd_sys_cfg_2040_coex(int argc, char *argv[])
{
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            if (argc)
                printf
                    ("20/40 coex configuration is allowed to set only before bss start.\n");
            ret = UAP_FAILURE;
        }
    } else {
        printf("ERR:Command sending failed!\n");
...
apcmd_sys_cfg_11n(int argc, char *argv[])
{
    ...
    if (argc) {
        if ((ISDIGIT(argv[0]) == 0) || (atoi(argv[0]) < 0) ||
            (atoi(argv[0]) > 1)) {
            printf
                ("ERR: Illegal ENABLE parameter %s. Must be either '0' or '1'.\n",
                 argv[0]);
            print_sys_cfg_11n_usage();
            return UAP_FAILURE;
        }
    ...
    if (!argc) {
        ...
        ht_info_tlv =
            (tlvbuf_htinfo_t *) (buffer + sizeof(apcmdbuf_sys_configure) +
                                 TLVHEADER_LEN);
        ...
        if (atoi(argv[0]) == 0) {
            ...
            if (argc == 5) {
                supported_mcs_set = (t_u32) A2HEXDECIMAL(argv[4]);
...
apcmd_sys_cfg_wmm(int argc, char *argv[])
{
    ...
    if (wmm_disable) {
        ...
        /* Initialize access categories (i.e. ACI field) */
        for (i = 0; i < MAX_AC_QUEUES; i++) {
            tlv->wmm_para.ac_params[i].aci_aifsn.aci = (t_u8) i;
        }
    } else {
        if (flag) {
            if (qos_info)
