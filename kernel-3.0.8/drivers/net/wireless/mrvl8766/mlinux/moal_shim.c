                      ...
                      IN pmlan_buffer pmbuf, IN t_u32 port)
{
    ...
    if (handle->is_suspended == MTRUE) {
        PRINTM(MERROR, "write_data_async is not allowed while suspended\n");
        LEAVE();
        return MLAN_STATUS_FAILURE;
    }
    ret =
...
moal_recv_packet(IN t_void * pmoal_handle, IN pmlan_buffer pmbuf)
{
    ...
    if (pmbuf) {
        ...
        if (cardp->rx_deaggr_ctrl.enable) {
            max_rx_data_size = cardp->rx_deaggr_ctrl.aggr_max;
            if (cardp->rx_deaggr_ctrl.aggr_mode == MLAN_USB_AGGR_MODE_NUM) {
                max_rx_data_size *= MAX(MLAN_USB_MAX_PKT_SIZE,
                                        cardp->rx_deaggr_ctrl.aggr_align);
                max_rx_data_size =
                    MAX(max_rx_data_size, MLAN_USB_RX_DATA_BUF_SIZE);
            }
        }
#endif
        priv = woal_bss_index_to_priv(pmoal_handle, pmbuf->bss_index);
    ...
    case MLAN_EVENT_ID_FW_DISCONNECTED:
        ...
        if (priv->reassoc_on == MTRUE) {
            if (priv->auto_assoc_priv.
                auto_assoc_type_on & (0x1 << (AUTO_ASSOC_TYPE_DRV_RECONN - 1)))
            {
                PRINTM(MINFO, " auto assoc: trigger driver auto re-connect\n");
...
#endif
        woal_broadcast_event(priv, CUS_EVT_HEART_BEAT,
                             strlen(CUS_EVT_HEART_BEAT));
        break;
