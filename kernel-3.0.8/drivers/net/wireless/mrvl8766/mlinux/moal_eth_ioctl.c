...
woal_priv_assocessid(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
    ...
    priv->auto_assoc_priv.auto_assoc_trigger_flag = AUTO_ASSOC_TYPE_DRV_ASSOC;
    priv->auto_assoc_priv.drv_assoc.status = MTRUE;
    if (priv->auto_assoc_priv.
        auto_assoc_type_on & (0x1 << (AUTO_ASSOC_TYPE_DRV_ASSOC - 1))) {
 ...
 */
int
woal_priv_setgetautoassoc(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
#ifdef REASSOCIATION
    moal_handle *handle = priv->phandle;
#endif
    ...
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_AUTOASSOC))) {
        PRINTM(MERROR, "No argument, invalid operation!\n");
        ret = -EINVAL;
        goto done;
    } else {
        ...
        memset((char *) data, 0, sizeof(data));
        parse_arguments(respbuf + strlen(CMD_MARVELL) +
                        strlen(PRIV_CMD_AUTOASSOC), data,
                        sizeof(data) / sizeof(int), &user_data_len);
    }
    ...
    if (user_data_len == 1) {
        ...
        if (data[0] == AUTO_ASSOC_TYPE_FW_RECONN) {
            ...
            if (MLAN_STATUS_SUCCESS !=
                ...
                data[2] = misc->param.fw_auto_reconnect.fw_reconn_counter;
                if (data[2] == 0) {
                    data[1] = 0;
                    sprintf(respbuf, "%d %d", data[0], data[1]);
                    ret = strlen(respbuf) + 1;
                    ...
                    data[1] = 1;
                    data[3] = misc->param.fw_auto_reconnect.fw_reconn_interval;
                    data[4] = misc->param.fw_auto_reconnect.fw_reconn_flags;
                    sprintf(respbuf, "%d %d 0x%x 0x%x 0x%x", data[0], data[1],
            ...
            if (priv->auto_assoc_priv.
                auto_assoc_type_on & (0x1 << (data[0] - 1))) {
                data[1] = 1;
                if (data[0] == AUTO_ASSOC_TYPE_DRV_RECONN) {
                    /* Get driver auto re-connect parameters */
                    data[2] = priv->auto_assoc_priv.drv_reconnect.retry_count;
                    data[3] =
                        priv->auto_assoc_priv.drv_reconnect.retry_interval;
                } else {
                    /* Get driver auto assoc parameters */
                    data[2] = priv->auto_assoc_priv.drv_assoc.retry_count;
                    data[3] = priv->auto_assoc_priv.drv_assoc.retry_interval;
                }
                sprintf(respbuf, "%d %d 0x%x 0x%x", data[0], data[1], data[2],
                        data[3]);
                ret = strlen(respbuf) + 1;
            } else {
                data[1] = 0;
        ...
        if (data[1] == MFALSE) {
            ...
            priv->auto_assoc_priv.auto_assoc_type_on &= ~(0x1 << (data[0] - 1));
            ...
            if (user_data_len > 2) {
                ...
                if (user_data_len > 3) {
                    ...
                    data[3] = 0xa;      // Default retry interval: 10 seconds
                }
            } else {
                data[2] = 0xff; // Default retry count: retry forever
                data[3] = 0xa;  // Default retry interval: 10 seconds
            }
            priv->auto_assoc_priv.auto_assoc_type_on |= 0x1 << (data[0] - 1);
        } else {
            PRINTM(MERROR, "Invalid auto assoc on/off option = %u\n", data[1]);
            ret = -EINVAL;
            goto done;
        }
        ...
        if (data[0] == AUTO_ASSOC_TYPE_FW_RECONN) {
            ...
            if (data[1] == MFALSE) {
                ...
                misc->param.fw_auto_reconnect.fw_reconn_counter = 0;
                misc->param.fw_auto_reconnect.fw_reconn_interval = 0;
                misc->param.fw_auto_reconnect.fw_reconn_flags = 0;
            } else {
                /* Set fw auto re-connect Enable */
                misc->param.fw_auto_reconnect.fw_reconn_counter = data[2];
                misc->param.fw_auto_reconnect.fw_reconn_interval = data[3];
                misc->param.fw_auto_reconnect.fw_reconn_flags = data[4];
            }
...
#endif
                priv->auto_assoc_priv.drv_reconnect.retry_count = data[2];
                priv->auto_assoc_priv.drv_reconnect.retry_interval = data[3];
            }
            if (data[0] == AUTO_ASSOC_TYPE_DRV_ASSOC) {
...
#endif
                priv->auto_assoc_priv.drv_assoc.retry_count = data[2];
                priv->auto_assoc_priv.drv_assoc.retry_interval = data[3];
            }
        }
        ret = sprintf(respbuf, "OK\n") + 1;
    }
                              ...
                              t_u32 respbuflen)
{
    ...
    memset((char *) data, 0, sizeof(data));
    if (strlen(respbuf) == (strlen(CMD_MARVELL) + strlen(PRIV_CMD_USBAGGRCTRL))) {
        /* GET operation */
        user_data_len = 0;
    } else {
        /* SET operation */
    ...
    if (user_data_len == 0) {
        memcpy(respbuf, (t_u8 *) & pcfg_misc->param.usb_aggr_params,
               sizeof(mlan_ds_misc_usb_aggr_ctrl));
    ...
    case 7:
        if (data[6] < 0 ||
            (data[6] > 10000 && data[6] != MLAN_USB_TX_AGGR_TIMEOUT_DYN)) {
    ...
    case 4:
        if ((data[3] == 2) || (data[3] == 4) || (data[3] == 8) ||
            (data[3] == 16)) {
            pcfg_misc->param.usb_aggr_params.rx_deaggr_ctrl.aggr_mode =
                MLAN_USB_AGGR_MODE_NUM;
        } else if ((data[3] == 4096) || (data[3] == 8192) || (data[3] == 16384)
                   || (data[3] == 32768)) {
            pcfg_misc->param.usb_aggr_params.rx_deaggr_ctrl.aggr_mode =
    ...
    case 3:
        if ((data[2] == 2) || (data[2] == 4) || (data[2] == 8) ||
            ...
            pcfg_misc->param.usb_aggr_params.tx_aggr_ctrl.aggr_mode =
                MLAN_USB_AGGR_MODE_NUM;
        } else if ((data[2] == 4096) || (data[2] == 8192) || (data[2] == 16384)
                   || (data[2] == 32768)) {
            pcfg_misc->param.usb_aggr_params.tx_aggr_ctrl.aggr_mode =
                MLAN_USB_AGGR_MODE_LEN;
        } else {
    ...
    case 1:
        ...
        pcfg_misc->param.usb_aggr_params.tx_aggr_ctrl.enable = (t_u16) data[0];
    default:
        break;
    }
    ...
    memcpy(&cardp->tx_aggr_ctrl, &pcfg_misc->param.usb_aggr_params.tx_aggr_ctrl,
           sizeof(usb_aggr_ctrl));

    if (usb_resubmit_urbs) {
        /* Indicate resubmition from here */
        cardp->resubmit_urbs = 1;
        /* Rx SG parameters has changed or disabled, kill the URBs, they will
           be resubmitted after saving the parameters to USB card */
...
woal_priv_sysclock(moal_private * priv, t_u8 * respbuf, t_u32 respbuflen)
{
    ...
    if (user_data_len) {
        ...
        cfg->param.sys_clock.sys_clk_num = MIN(MLAN_MAX_CLK_NUM, user_data_len);
        ...
        length_index = MIN(cfg->param.sys_clock.sys_clk_num, MLAN_MAX_CLK_NUM);

        /* Configurable clocks */
        for (i = 1; i <= length_index; i++) {
            data[i + data_length] = (int) cfg->param.sys_clock.sys_clk[i - 1];
        }
...
#define CMD_BUF_LEN   2048
    ...
    if (strncmp(buf, CMD_MARVELL, strlen(CMD_MARVELL)) == 0) {
        ...
        if (strnicmp
            ...
            if (strnicmp
                (buf + strlen(CMD_MARVELL), PRIV_CMD_AUTOASSOC,
                 strlen(PRIV_CMD_AUTOASSOC)) == 0) {
            /* Auto assoc */
            len = woal_priv_setgetautoassoc(priv, buf, priv_cmd.total_len);
            goto handled;
        } else
            if (strnicmp
            ...
            if (strnicmp
                (buf + strlen(CMD_MARVELL), PRIV_CMD_USBAGGRCTRL,
                 strlen(PRIV_CMD_USBAGGRCTRL)) == 0) {
            /* USB aggregation control */
            len = woal_priv_set_get_usbaggrctrl(priv, buf, priv_cmd.total_len);
            goto handled;
        } else
            if (strnicmp
