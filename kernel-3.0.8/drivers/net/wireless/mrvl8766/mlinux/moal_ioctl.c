...
woal_copy_mc_addr(mlan_multicast_list * mlist, mlan_802_11_mac_addr mac)
{
    ...
    if (mlist->num_multicast_addr < MLAN_MAX_MULTICAST_LIST_SIZE)
        memcpy(&mlist->mac_list[mlist->num_multicast_addr], mac, ETH_ALEN);
...
#endif
                if (mc_count)
                    woal_copy_mcast_addr(mlist, priv->netdev);
            }
        }
#endif
    }
...
woal_request_set_multicast_list(moal_private * priv, struct net_device *dev)
{
    ...
    if (dev->flags & IFF_PROMISC) {
        ...
        mc_count =
            woal_copy_all_mc_list(priv->phandle, &bss->param.multicast_list);
...
woal_tdls_config_ioctl(struct net_device *dev, struct ifreq *req)
{
    ...
    if (!(tdls_data = kmalloc(sizeof(mlan_ds_misc_tdls_config), GFP_KERNEL))) {
        ret = -ENOMEM;
        goto done;
    }
...
woal_usb_aggr_init(moal_handle * handle)
{
    ...
    if (!handle || !(priv = woal_get_priv(handle, MLAN_BSS_ROLE_STA))) {
        PRINTM(MERROR, "Handle or STA priv is null\n");
        ret = MLAN_STATUS_FAILURE;
        goto done;
    }
    if (handle->card == NULL) {
    ...
    if (usb_aggr == 0 || usb_aggr == 2) {
        usb_aggr_enable = MFALSE;
    } else if (usb_aggr == 1) {
        usb_aggr_enable = MTRUE;
    } else {
        PRINTM(MWARN, "Invalid module param (usb_aggr) value %d, "
               "using MLAN default\n", usb_aggr);
        usb_aggr = 0;
        usb_aggr_enable = MFALSE;
    }
    ...
    aggr_param->tx_aggr_ctrl.aggr_tmo = MLAN_USB_TX_AGGR_TIMEOUT_MSEC * 1000;
    ...
    if (req->status_code)
        ...
        memcpy(&cardp->tx_aggr_ctrl, &aggr_param->tx_aggr_ctrl,
    ...
    if (resubmit_urbs) {
        ...
        /* Rx SG feature is disabled due to FW failure/unsupported, kill the
           URBs, they will be resubmitted after saving the parameters to USB
           card */
        ...
        memcpy(&cardp->rx_deaggr_ctrl, &aggr_param->rx_deaggr_ctrl,
               sizeof(usb_aggr_ctrl));
