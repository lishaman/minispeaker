                           ...
                           struct cfg80211_connect_params *conn_param)
{
    ...
    memcpy(scan_req.ssid_list[0].ssid, conn_param->ssid, conn_param->ssid_len);
    scan_req.ssid_list[0].max_len = 0;
    if (conn_param->channel) {
        scan_req.chan_list[0].chan_number = conn_param->channel->hw_value;
        scan_req.chan_list[0].radio_type = conn_param->channel->band;
        if (conn_param->channel->
            flags & (IEEE80211_CHAN_PASSIVE_SCAN | IEEE80211_CHAN_RADAR))
        ...
        for (band = 0; (band < IEEE80211_NUM_BANDS); band++) {
            ...
            for (i = 0; (i < sband->n_channels); i++) {
                ...
                scan_req.chan_list[chan_idx].chan_number = (u32) ch->hw_value;
                chan_idx++;
            }
        }
    }
...
woal_cfg80211_assoc(moal_private * priv, void *sme)
{
    ...
    if (MLAN_STATUS_SUCCESS != woal_find_essid(priv, &ssid_bssid)) {
        ...
        if (mode != MLAN_BSS_MODE_IBSS)
            ...
            ret = woal_request_scan(priv, MOAL_IOCTL_WAIT, &req_ssid);
        if (ret) {
            ret = -EFAULT;
            goto done;
        }
