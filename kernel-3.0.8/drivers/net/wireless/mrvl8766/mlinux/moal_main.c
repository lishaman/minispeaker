...
/** Driver version */
char driver_version[] =
    "USB8766-%s-M" KERN_VERSION "14" MLAN_RELEASE_VERSION
    "-GPL" "-(" "FP" FPNUM ")"
#ifdef	DEBUG_LEVEL2
    "-dbg"
#endif
...
#define USB_WAIT_FW_READY  (500)
                ...
                if ((usbRstDev_ret == 0) || (usbRstDev_ret == -ENODEV) ||       // expected 
                                                                                ...
                                                                                // chip 
                                                                                // re-enumerates
                    (usbRstDev_ret == -EINVAL)) {       // expected if USB FW
                                                        // detaches first
...
#define MAX_WMM_QUEUE   4
    ...
    PRINTM(MERROR, "%s: Marvell 802.11 Adapter\n", dev->name);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,24)
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,34)
    /* Setting 'pm_usage_count' to 1; Subsequently calling *_put_interface
       will set it to 0. */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,32)
    intf->pm_usage_cnt = 1;
#else
...
#endif
        priv->auto_assoc_priv.auto_assoc_type_on = 0;
        priv->auto_assoc_priv.auto_assoc_trigger_flag = AUTO_ASSOC_TYPE_NONE;
        memset(&priv->auto_assoc_priv.drv_assoc, 0, sizeof(drv_auto_assoc));
        memset(&priv->auto_assoc_priv.drv_reconnect, 0, sizeof(drv_auto_assoc));
    }
#endif /* STA_SUPPORT */
#ifdef UAP_SUPPORT
...
woal_reset_intf(moal_private * priv, t_u8 wait_option, int all_intf)
{
    ...
    if (!all_intf) {
        ...
        for (intf_num = 0; intf_num < handle->priv_num; intf_num++) {
            ...
            handle->priv[intf_num]->auto_assoc_priv.drv_assoc.status = MFALSE;
            handle->priv[intf_num]->auto_assoc_priv.drv_reconnect.status =
                MFALSE;
...
#endif
    if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_STA) {
        /* Check if heart beat set */
        if ((priv->is_h2d_active == MTRUE) || (priv->is_d2h_active == MTRUE)) {
            ...
            if (req) {
                ...
                misc->param.heart_beat.reconnect_delay = HB_CFG_DISABLE;
                if (MLAN_STATUS_SUCCESS !=
                    woal_request_ioctl(priv, req, wait_option)) {
                    ret = -EFAULT;
                    kfree(req);
                    goto done;
                }
    ...
    for (;;) {
        ...
        for (i = 0; i < handle->priv_num && (priv = handle->priv[i]); i++) {
            ...
            if (priv->auto_assoc_priv.auto_assoc_trigger_flag ==
                ...
                if (priv->auto_assoc_priv.auto_assoc_trigger_flag ==
                    ...
                    retry_interval =
                        ...
                        1000;
                    if (retry_count == 0xff)
                        retry_count = AUTO_ASSOC_RETRY_FOREVER;
                    priv->auto_assoc_priv.drv_reconnect.status = MTRUE;
                    PRINTM(MINFO,
            ...
            if (priv->auto_assoc_priv.drv_assoc.status == MTRUE) {
                /* Search AP by ESSID for driver auto reassoc */
                PRINTM(MINFO, "Reassoc: Search AP by ESSID\n");
        ...
        if (reassoc_timer_req == MTRUE) {
            ...
            if (priv->auto_assoc_priv.drv_assoc.status == MTRUE ||
                ...
                if (retry_count != AUTO_ASSOC_RETRY_FOREVER)
                    retry_count--;
                woal_mod_timer(&handle->reassoc_timer, retry_interval);
            } else {
                PRINTM(MEVENT,
...
woal_main_work_queue(struct work_struct * work)
{
    ...
    if (atomic_read(&cardp->rx_data_urb_pending) < MVUSB_RX_DATA_URB / 2) {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,36)
    handle->workqueue =
        alloc_workqueue("MOAL_WORK_QUEUE", WQ_HIGHPRI | WQ_MEM_RECLAIM, 1);
#else
