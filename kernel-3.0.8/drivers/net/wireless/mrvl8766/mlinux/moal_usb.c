...
struct usb_device_id woal_usb_table[] = {
    ...
    {MARVELL_USB_DEVICE
     (USB8766_VID_1, USB8766_PID_1, "Marvell WLAN USB Adapter")},
    {MARVELL_USB_DEVICE
     (USB8766_VID_1, USB8766_PID_2, "Marvell WLAN USB Adapter")},
    /* Terminating entry */
    {},
};
...
woal_usb_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    for (i = 0; woal_usb_table[i].idVendor; i++) {
        if (woal_cpu_to_le16(udev->descriptor.idVendor) ==
            ...
            printk("woal_usb_table[i].idVendor)=%#x i = %d\n", woal_usb_table[i].idVendor, i);           
            break;
        }
    }
    ...
    if (woal_usb_table[i].idVendor) {
        ...
        PRINTM(MMSG, "bcdUSB = 0x%X bDeviceClass = 0x%X"
        ...
        for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
            ...
            if (usb_endpoint_is_bulk_in(endpoint) &&
                ...
                PRINTM(MMSG, "Bulk IN: max packet size = %d, address = %d\n",
            ...
            if (usb_endpoint_is_bulk_out(endpoint) &&
                ...
                PRINTM(MMSG, "Bulk OUT: max packet size = %d, address = %d\n",
                       woal_le16_to_cpu(endpoint->wMaxPacketSize),
                       endpoint->bEndpointAddress);
...
woal_usb_submit_rx_data_urbs(moal_handle * handle)
{
    ...
    int i;
    mlan_status ret = MLAN_STATUS_FAILURE;
    t_u32 buffer_len = MLAN_USB_RX_DATA_BUF_SIZE;
