...
#ifdef CONFIG_LED
#define LED_RECORD  GPIO_PC(2)
#define LED_WIFI    GPIO_PC(3)
#endif
...
int checkboard(void)
{
	puts("Board: trunk (Ingenic XBurst JZ4775 SoC)\n");
	return 0;
}
...
{
#ifdef CONFIG_LED
	gpio_request(LED_RECORD, "led-record");
	gpio_request(LED_WIFI, "led-wifi");
	gpio_direction_output(LED_RECORD, 0);
	gpio_direction_output(LED_WIFI, 0);
#endif
}
